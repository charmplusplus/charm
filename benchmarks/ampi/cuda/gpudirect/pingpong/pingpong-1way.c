...
int main(int argc, char** argv)
{
  ...
  int max_msg_size, min_msg_size, msg_size, low_iter, high_iter, iter, printFormat, gpu;
  MPI_Init(&argc, &argv);
  ...
  if (!gpu) {
    ...
    hapiCheck(cudaMalloc((void**)&message_s, max_msg_size));
    hapiCheck(cudaMalloc((void**)&message_r, max_msg_size));
  }
  ...
  if (!gpu) {
    ...
    hapiCheck(cudaFree(message_s));
    hapiCheck(cudaFree(message_r));
  }
