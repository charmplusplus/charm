...
public:
  ...
  main(CkArgMsg* m)
  {
    ...
    buffer = (char *)CkRdmaAlloc(sizeof(char) * maxSize);
  ...
  void regular_bcast_done() {
    ...
    if(niter == iterations) {
      ...
      niter = 0;
      start_time = CkWallTimer();
      ngid.recv_zerocopy(CkSendBuffer(buffer, sendDoneCb, CK_BUFFER_PREREG), size, niter, warmUp, iterations);
    } else {
      ngid.recv(buffer, size, niter, warmUp, iterations);
    }
  }
  ...
  void zc_send_done() {
    ...
    if(send_counter == 2) {
      ...
      if(niter == iterations) {
        ...
        start_time = CkWallTimer();
        ngid.recv_zerocopy_post(CkSendBuffer(buffer, recvDoneCb, CK_BUFFER_PREREG), size, niter, warmUp, iterations);
      } else {
        ngid.recv_zerocopy(CkSendBuffer(buffer, sendDoneCb, CK_BUFFER_PREREG), size, niter, warmUp, iterations);
      }
    }
...
public:
  LargeDataNodeGroup(int maxSize) {
    ...
    myBuffer = (char *)CkRdmaAlloc(sizeof(char) * maxSize);
  }
