...
public:
  ...
  void regular_bcast_done() {
    ...
    if(niter == iterations) {
      ...
      niter = 0;
      start_time = CkWallTimer();
      ngid.recv_zerocopy(CkSendBuffer(buffer, sendDoneCb), size, niter, warmUp, iterations);
    } else {
      ngid.recv(buffer, size, niter, warmUp, iterations);
    }
  }
  ...
  void zc_send_done() {
    ...
    if(send_counter == 2) {
      ...
      if(niter == iterations) {
        ...
        start_time = CkWallTimer();
        ngid.recv_zerocopy_post(CkSendBuffer(buffer, recvDoneCb), size, niter, warmUp, iterations);
      } else {
        ngid.recv_zerocopy(CkSendBuffer(buffer, sendDoneCb), size, niter, warmUp, iterations);
      }
    }
...
public:
  ...
  void recv_zerocopy_post(char *&msg, size_t &size, int iter, bool warmUp, int iterations, CkNcpyBufferPost *) {
    msg = myBuffer;
  }
