	...
	int dist(int i1, int i2, int i3){
	int x1 = abs(i1-i2);
	int x2 = 1+ abs(i3-1+i1-i2);
	int x3 = 1+ abs(-i3 +1 +i1-i2);
//	int x1 =(abs(i3+i1-i2)%i3);
//	int x2 =(abs(i3+i2-i1)%i3);
	int x = min(x2,x3);
	x = min(x,x1);
	return x;
	}
	...
	*/
	pair<int,vector<double> > EventInterpolator::parseParameters(const string &funcname, istringstream &line, int stepNo){
	...
	if( funcname == string("calc_self_energy_merge_fullelect") ||
		...
		params.push_back( min(d1,d2));
					params.push_back( max(d1,d2));
					int dx = dist(d3,d6,numX);
					int dy = dist(d4,d7,numY);
					int dz = dist(d5,d8,numZ);

					if(dx>2)
					{
							cout<<" Problem Case for dist:  " << d3 <<"  "<<d4<<"   "<<numX<<"    "<<dx<<endl;
					}
		params.push_back( dx );
		params.push_back( dy );
		params.push_back( dz );
		params.push_back( d9 );
		params.push_back( d10 );
		params.push_back( d11 );
		params.push_back(stepNo);
	}
	else { /** @note default model is quadratic in each variable */
	if(funcname == string("angles") ||
					funcname == string("dihedrals"))
	{
		double d, d1;
		int count=0;
		while(line.good()){
	...
	void EventInterpolator::LoadTimingsFile(const char *table_filename){
...
//	if(temp == string("TRACEBIGSIM:")){
	...
	while(line.good() && accurateTimeTable.good()){
		...
		} else if (field == string("time_in_us:{")){
		line >> time;
		line >> temp; // gobble up the '}'
		if(temp != string("}"))
		cerr << "ERROR: malformed entry in expected times file" << endl;
	...
	if(funcname != string("") && time != -1){
		...
		fullParams params = parseParameters(funcname,paramstream, 1);
...
//	}
	...
	*/
	...
	while(parameterEventTable.good()){
		...
		if(parameterEventTable.good()){
			...
			if(t1 == string("TRACEBIGSIM:")){
				...
				while(line.good() && parameterEventTable.good()){
					//cout<< "Parsing the parameters: "<<endl;
					if(field == string("step:{")){
						line >> stepNo;
						line >> temp; // gobble up the '}'
					if(temp != string("}"))
						cerr << "ERROR: malformed entry in expected times file" << endl;
					} 
					else if(field == string("seqno:{")){
						line >> seqNo;
						line >> temp; // gobble up the '}'
					}
					else if (field == string("params:{")){
			...
			istringstream paramstream(paramstring);
			fullParams params = parseParameters(funcname,paramstream, stepNo);
			////params.second.pushback(stepNo);
			funcIdentifier func(funcname,params.first);
	...
	void EventInterpolator::LoadModel(){
	// Load in Parameter File which maps event id to function name and parameters
	cout << "Loading Model" << endl;
	char name[50];
	sprintf(name,"model");
	ifstream modelTable(name);
	if(modelTable.good()){
	#ifdef DEBUG
	cout << "     >  Loading " << name << endl;
	#endif
		...
		istringstream line(line_s);
						...
						if(modelTable.good())
						{
							...
							cout<<" read funcname : "<< funcname  << "  c1:  "<< c1<<"   t2:  "<<c2;
							temp.push_back(c1);
							temp.push_back(c2);
							temp.push_back(c3);
							temp.push_back(c4);
							...
							temp.push_back(c8);
							temp.push_back(c9);
							model[make_pair(funcname,0)] = temp;
							temp.clear();
			}
			}
			}
	...
	EventInterpolator::EventInterpolator(const char *table_filename, double sample_rate) :
	exact_matches(0),
	...
	double EventInterpolator::haveNewTiming(const unsigned pe, const unsigned eventid) {
			//cout<< " looking for "<<pe <<"   "<<eventid<<endl;
	...
	const pair<funcIdentifier,vector<double> > p = eventparams[make_pair(pe,eventid)];
	double newTime;		
	double secondTime;		
	if(haveExactTime(pe,eventid) )
	{
			newTime= lookupExactTime(p.first,p.second);
	    //secondTime = predictTime(p.first,p.second);
      //cout<<" Exact Time " << newTime<< " second Time"<< secondTime<<endl;
	}
	else
		newTime= predictTime(p.first,p.second);

		if(p.second.back()>4)
		{
  			string funcname = p.first.first;
			  //cout<<" Step No. " <<p.second.back()<< "   func name   "<< funcname<< endl;
				/*if(funcname == string("angles"))
				{
						cout<<"Sum of angle timings is "<<sum_newTiming[p.first]<<endl; 
				}*/
				sum_newTiming[p.first] += newTime; 
		}
	
	return newTime*0.000001;  // Convert back to seconds
	}
		...
		funcname == string("calc_pair_energy") )
			{
					...
					for(int i=0;i<8;++i)
					{
					sum = sum + temp[i]*params[i];
				//	cout<<params[i]<<"  "<<temp[i]<<"  "<<sum<<" ;  ";
					}
					sum = sum +temp[8];
					//cout<<sum<<endl;
			}
			...
			{	
					sum = temp[0] *params[0]+ temp[1] *params[1] + temp[2]*params[1]*params[1]+ temp[3];
					//cout<<"Func "<<funcname<<"   "<<sum<<endl;
			}
	approx_matches++;
	return sum;
	}
	...
	for(map<funcIdentifier,double>::iterator i=sum_newTiming.begin();i!=sum_newTiming.end();++i){
		cout << "   > Sum of predicted/interpolated times for function " << (*i).first.first << "," << (*i).first.second << " is ";
		printf("%f  \n",(*i).second);
	}
	}
