...
public:
  ...
  void recvNcpyInfo(CkNcpyBuffer dest)
  {
    ...
    // CkNcpyCallbackMode::CB_INVOKE_NONINLINE is set to invoke sender callback only in non-inline transfers (async RDMA call)
    CkNcpyStatus status = mySrc.put(dest, CkNcpyCallbackMode::CB_INVOKE_NONINLINE);
    if(status == CkNcpyStatus::complete) {
      CkPrintf("[%d][%d][%d] Inline                     : Put is complete - ", thisIndex, CkMyPe(), CkMyNode());
      CkPrintf("De-registering and freeing source data\n");
      // de-register & free buffer as Put has completed
      mySrc.deregisterMem();
      delete [] iArr;
      mainProxy.maindone();
    }
