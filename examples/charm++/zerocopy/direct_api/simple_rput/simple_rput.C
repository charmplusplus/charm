...
public:
  ...
  void start()
  {
    ...
    // Send my targets to Index 0; Index 0 performs Rputs into these targets
    thisProxy[otherIndex].recvNcpyInfo(myTar1, myTar2, myTar3);
  }
  ...
  void putReceiverDone(CkDataMsg *m){
    ...
    if(cbCounter == 3) {
      ...
      CkPrintf("[%d][%d][%d] Rput Target Done\n", thisIndex, CkMyPe(), CkMyNode());
  ...
  // Executed on Index 0
  void recvNcpyInfo(CkNcpyTarget tar1, CkNcpyTarget tar2, CkNcpyTarget tar3)
  {
    CkAssert(thisIndex == 0);
    ...
    // Perform Rputs from my sources into Index 1's targets
    mySrc1.rput(tar1);
    mySrc2.rput(tar2);
    mySrc3.rput(tar3);
  }
