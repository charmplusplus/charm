...
export LD_LIBRARY_PATH="/u/bhosale/charm-reconverse/multicore-linux-x86_64-cuda/lib:/u/bhosale/lci_install/lib64:$LD_LIBRARY_PATH" IPATH_NO_BACKTRACE=1 FI_MR_CACHE_MONITOR=disabled CHARM_ZC_STATS=1 BARNES_MASS_CHECK=1 PMI_MAX_KVS_ENTRIES=1024
NODES=${NODES:-1}; NT=$((NODES * 4)); P=$((NT * 4))
...
for r in $(seq 1 $REPS); do for tag in "$@"; do
  ...
  PES=4 timeout 200 srun --unbuffered --jobid=$J --mpi=cray_shasta -N $NODES -n $NT --ntasks-per-node=4 --cpus-per-task=8 --cpu-bind=none --exact stdbuf -oL -eL ./numa_wrap.sh ./barnes $BASE +p 4 $LB $IPC > $L 2>&1; rc=$?; sleep 1
