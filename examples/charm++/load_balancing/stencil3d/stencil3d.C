...
public:
  ...
  Main(CkArgMsg* m)
  {
    if ((m->argc != 3) && (m->argc != 7))
    {
      ...
      CkPrintf(
          "OR %s [array_size_X] [array_size_Y] [array_size_Z] [block_size_X] "
          "[block_size_Y] [block_size_Z]\n",
          m->argv[0]);
      CkAbort("Abort");
    }
    ...
    CkPrintf("Running Stencil on %d processors with (%d, %d, %d) chares\n", CkNumPes(),
             num_chare_x, num_chare_y, num_chare_z);
...
public:
  ...
  void compute_kernel()
  {
    ...
    int index =
        thisIndex.x + thisIndex.y * num_chare_x + thisIndex.z * num_chare_x * num_chare_y;
  ...
  void constrainBC()
  {
    ...
    for (int k = 1; k < blockDimZ + 1; ++k)
      for (int i = 1; i < blockDimX + 1; ++i) temperature[index(i, 1, k)] = 255.0;
    for (int k = 1; k < blockDimZ + 1; ++k)
      for (int j = 1; j < blockDimY + 1; ++j) temperature[index(1, j, k)] = 255.0;
    for (int j = 1; j < blockDimY + 1; ++j)
