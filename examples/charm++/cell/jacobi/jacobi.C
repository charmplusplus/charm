...
Jacobi::~Jacobi()
{
  ...
  if (matrix != NULL) {
    CmiFreeAligned((void*)matrix); matrix = NULL;
  }

  if (matrixTmp != NULL) {
    CmiFreeAligned((void*)matrixTmp); matrixTmp = NULL;
  }
...
void Jacobi::startIteration()
{
  ...
  #if USE_MESSAGES != 0
  if (futureNorthMsg != NULL) {
    northData_msg(futureNorthMsg); futureNorthMsg = NULL;
  }

  if (futureSouthMsg != NULL) {
    southData_msg(futureSouthMsg); futureSouthMsg = NULL;
  }

  if (futureEastMsg != NULL) {
    eastData_msg(futureEastMsg); futureEastMsg = NULL;
  }

  if (futureWestMsg != NULL) {
    westData_msg(futureWestMsg); futureWestMsg = NULL;
  }
  #endif
}
