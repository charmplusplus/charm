  ...
  public:
    ...
    void startIter() {
      ...
      for (int i = 0; i < N_SIZES; i++) {
        ...
        // of mempool
        if (use_mempool) {
          h_A[i] = (float*)hapiPoolMalloc(sizes[i]);
          h_B[i] = (float*)hapiPoolMalloc(sizes[i]);
          h_C[i] = (float*)hapiPoolMalloc(sizes[i]);
        }
        else {
          hapiCheck(cudaMallocHost((void**)&h_A[i], sizes[i]));
          hapiCheck(cudaMallocHost((void**)&h_B[i], sizes[i]));
          hapiCheck(cudaMallocHost((void**)&h_C[i], sizes[i]));
        }
