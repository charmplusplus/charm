...
// NOTE: The numbers for NUM_ROWS, NUM_COLS, and NUM_CHARES below are the  numbers
//   that were used for testing GPGPUs (CUDA) and Cell respectively.  The numbers
//   for Cell are lower because the Offload API has an yet-to-be-implemented
//   enhancement that would allow parameters >16k bytes in size.  Until that is
//   add/fixed, the tile size is limited when executing on the Cell processor.
#define NUM_ROWS      128  // ( GPGPU: 128, Cell: 58 )  // The number of data rows each chare has
#define NUM_COLS      130  // ( GPGPU: 130, Cell: 62 )   // The number of data columns each chare has (vectorized code used on SPE if this is a '(multiple of 4) +/- 2')
#define NUM_CHARES     32  // ( GPGPU:  32, Cell: 24 )  // The number of chares (per dimension)
