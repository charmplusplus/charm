...
extern void kernelSetup(hapiStream_t stream, const CkCallback& cb);
...
class Hello : public CBase_Hello {
 ...
 public:
  Hello() { hapiCheck(hapiStreamCreate(&stream)); }

  ~Hello() { hapiCheck(hapiStreamDestroy(stream)); }

  void greet() {
    int device;
    hapiCheck(hapiGetDevice(&device));
    hapiDeviceProp prop;
    hapiCheck(hapiGetDeviceProperties(&prop, device));
