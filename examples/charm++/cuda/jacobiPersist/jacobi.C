...
Jacobi::Jacobi() {
  ...
  tmpMaxError = (float*)(CmiMallocAligned(sizeof(float) * NUM_ROWS, 128));
  northGhostData = (float*)(CmiMallocAligned(sizeof(float) * NUM_COLS * 2, 128));
  southGhostData = (float*)(CmiMallocAligned(sizeof(float) * NUM_COLS * 2, 128));
  eastGhostData = (float*)(CmiMallocAligned(sizeof(float) * NUM_ROWS * 2, 128));
  westGhostData = (float*)(CmiMallocAligned(sizeof(float) * NUM_ROWS * 2, 128));

  // Initialize the data
  memset((void*)matrix, 0, sizeof(float) * DATA_BUFFER_SIZE);
  memset((void*)matrixTmp, 0, sizeof(float) * DATA_BUFFER_SIZE);
  memset((void*)tmpMaxError, 0, sizeof(float) * NUM_ROWS);
  memset((void*)northGhostData, 0, sizeof(float) * NUM_COLS * 2);
  memset((void*)southGhostData, 0, sizeof(float) * NUM_COLS * 2);
  memset((void*)eastGhostData, 0, sizeof(float) * NUM_ROWS * 2);
  memset((void*)westGhostData, 0, sizeof(float) * NUM_ROWS * 2);

  northGhostDataOut = northGhostData + NUM_COLS;
  southGhostDataOut = southGhostData + NUM_COLS;
  eastGhostDataOut = eastGhostData + NUM_ROWS;
  westGhostDataOut = westGhostData + NUM_ROWS;
  northGhostDataOut[0] = 1.0f;
  westGhostDataOut[0] = 1.0f;

  #if 1

  matrix[DATA_BUFFER_COLS + 1] = 1.0f;
  matrixTmp[DATA_BUFFER_COLS + 1] = 1.0f;
...
Jacobi::~Jacobi() {
  ...
  if (tmpMaxError != NULL) { CmiFreeAligned((void*)tmpMaxError); tmpMaxError = NULL; }

  if (northGhostData != NULL) { CmiFreeAligned((void*)northGhostData); northGhostData = NULL; }
  if (southGhostData != NULL) { CmiFreeAligned((void*)southGhostData); southGhostData = NULL; }
  if (eastGhostData != NULL) { CmiFreeAligned((void*)eastGhostData); eastGhostData = NULL; }
  if (westGhostData != NULL) { CmiFreeAligned((void*)westGhostData); westGhostData = NULL; }
...
void Jacobi::startIteration() {
  ...
  if (chareY > 0) {
      ...
      memcpy(northMsgSave[0]->data, northGhostDataOut, sizeof(float) * NUM_COLS);
      ...
      thisProxy[GET_CHARE_I(chareX, chareY - 1)].southData(NUM_COLS, northGhostDataOut, iterCount);
    #endif
  }
  ...
  if (chareY < (NUM_CHARES - 1)) {
      ...
      memcpy(southMsgSave[0]->data, southGhostDataOut, sizeof(float) * NUM_COLS);
      ...
      thisProxy[GET_CHARE_I(chareX, chareY + 1)].northData(NUM_COLS, southGhostDataOut, iterCount);
    #endif
  }
  ...
  if (chareX > 0) {
      ...
      memcpy(westMsgSave[0]->data, westGhostDataOut, sizeof(float) * NUM_ROWS);
      ...
      thisProxy[GET_CHARE_I(chareX - 1, chareY)].eastData(NUM_ROWS, westGhostDataOut, iterCount);
    #endif
  }
  ...
  if (chareX < (NUM_CHARES - 1)) {
      ...
      memcpy(eastMsgSave[0]->data, eastGhostDataOut, sizeof(float) * NUM_ROWS);
      ...
      thisProxy[GET_CHARE_I(chareX + 1, chareY)].westData(NUM_ROWS, eastGhostDataOut, iterCount);
    #endif
  }
...
void Jacobi::northData(int size, float* ghostData, int iterRef) {
  ...
  if (iterCount == iterRef) {  // If so, process it
    ...
    memcpy(northGhostData, ghostData, NUM_COLS * sizeof(float));
    attemptCalculation();
...
void Jacobi::northData_msg(NorthSouthGhost *msg) {
  ...
  if (msg->iterCount == iterCount) {  // If so, process it
    ...
    memcpy(northGhostData, msg->data, sizeof(float) * NUM_COLS);
...
void Jacobi::southData(int size, float* ghostData, int iterRef) {
  ...
  if (iterCount == iterRef) {  // If so, process it
    ...
    memcpy(southGhostData, ghostData, NUM_COLS * sizeof(float));
    attemptCalculation();
  } else {                     // If not, resend to self and try again later
...
void Jacobi::southData_msg(NorthSouthGhost *msg) {
  ...
  if (msg->iterCount == iterCount) {  // If so, process it
    ...
    memcpy(southGhostData, msg->data, sizeof(float) * NUM_COLS);
...
void Jacobi::eastData(int size, float* ghostData, int iterRef) {
  ...
  if (iterCount == iterRef) {  // If so, process it
    ...
    memcpy(eastGhostData, ghostData, sizeof(float) * NUM_ROWS);
    attemptCalculation();
  } else {                     // If not, resend to self and try again later
...
void Jacobi::eastData_msg(EastWestGhost* msg) {
  ...
  if (msg->iterCount == iterCount) {  // If so, process it
    ...
    memcpy(eastGhostData, msg->data, sizeof(float) * NUM_ROWS);
...
void Jacobi::westData(int size, float* ghostData, int iterRef) {
  ...
  if (iterCount == iterRef) {  // If so, process it
    ...
    memcpy(westGhostData, ghostData, sizeof(float) * NUM_ROWS);
    attemptCalculation();
  } else {                     // If not, resend to self and try again later
...
void Jacobi::westData_msg(EastWestGhost* msg) {
  ...
  if (msg->iterCount == iterCount) {  // If so, process it
    ...
    memcpy(westGhostData, msg->data, sizeof(float) * NUM_ROWS);
...
void Jacobi::doCalculation_post() {
  ...
  for (int i = 1; i < NUM_ROWS; i++) {
    ...
    mainProxy.reportMaxError(maxError, iterCount); 
  #endif
  ...
  // Display the matrix
    ...
    if (accelManager != NULL) {
      ...
      accelManager->pullPersistBuffer((void*)matrix);
      printf("matrix[%d] @ %p = {\n", thisIndex, matrix);
      for (int y = 0; y < DATA_BUFFER_ROWS; y++) {
        printf("  ");
        for (int x = 0; x < DATA_BUFFER_COLS; x++) {
          printf(" %e", matrix[GET_DATA_I(x,y)]);
        }
        printf("\n");
      }
      printf("}\n");

      accelManager->pullPersistBuffer((void*)matrixTmp);
      printf("matrixTmp[%d] @ %p = {\n", thisIndex, matrixTmp);
      for (int y = 0; y < DATA_BUFFER_ROWS; y++) {
        printf("  ");
        for (int x = 0; x < DATA_BUFFER_COLS; x++) {
          printf(" %e", matrixTmp[GET_DATA_I(x,y)]);
        }
        printf("\n");
      }
      printf("}\n");
