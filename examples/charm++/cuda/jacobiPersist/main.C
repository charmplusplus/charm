...
Main::Main(CkArgMsg *msg) {
  ...
  CkPrintf("  StepPerPrint: %d\n", DISPLAY_MAX_ERROR_FREQ);
  CkPrintf("  Num PEs: %d\n", CkNumPes());
  ...
  ftcb = new CkCallback(CkIndex_Main::startIteration(), thisProxy);
}
...
void Main::maxErrorReductionClient(CkReductionMsg *msg) {
  ...
  // DMK - DEBUG
  static double lastCallTime = -1.0;

  float maxError = *((float*)(msg->getData()));

  #if DISPLAY_MAX_ERROR_FREQ > 0
    if (iterationCount == 0 || (iterationCount % DISPLAY_MAX_ERROR_FREQ) == 0) {
      //CkPrintf("Iteration %d Finished @ %lf sec... maxError = %f...\n", iterationCount, CmiWallTimer(), maxError);
      double now = CmiWallTimer();
      double delta = now - ((lastCallTime < 0.0) ? (startTime) : (lastCallTime));
      lastCallTime = now;
      CkPrintf("Iteration %d Finished @ %lf sec (delta: %lf sec)... maxError = %f (%e)...\n",
               iterationCount, now, delta, maxError, maxError
              );
      fflush(NULL);
    }
  #endif
  ...
  if (maxError <= MAX_ERROR) {
    ...
    //// DMK - DEBUG
    if (iterationCount == 750) { traceBegin(); }
    if (iterationCount == 1050) { traceEnd(); }

    // DMK - DEBUG - Place in a timestep marker for the AccelManager, only if AccelManager isn't auto measuring
    AccelManager *manager = AccelManager::getAccelManager();
    if (manager != NULL && !(manager->getAutoMeasurePerf())) {
      manager->takePeriodicSample();
    }

    // DMK - Periodically checkpoint the application state
    if (iterationCount % 50 == 1) {  // NOTE: Checkpoint after first step
      CkStartMemCheckpoint(*ftcb);
    } else {
      jArray.startIteration();
    }
  }
