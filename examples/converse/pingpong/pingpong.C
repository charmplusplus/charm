...
void ringFinished(char* msg) {
  ...
  CmiPrintf(
      "Size=%d bytes, time=%lf microseconds one-way\n",
...
CmiHandler node0HandlerFunc(char* msg) {
  ...
  if (CpvAccess(cycleNum) == 1) CpvAccess(startTime) = CmiWallTimer();
...
CmiStartFn mymain(int argc, char* argv[]) {
  ...
  if (CmiMyPe() == 0) {
    CmiPrintf(
        "Pingpong with iterations = %d, minMsgSize = %d, maxMsgSize = %d, increase "
        "factor = %d\n",
