...
void operationFinished(char* msg) {
  CmiFree(msg);
  double cycle_time = (1e6 * (CpvAccess(endTime) - CpvAccess(startTime))) /
                      (1.0 * nCycles * (CpvAccess(kFactor) + 1));

  double compute_time = cycle_time - (1e6 * (CpvAccess(IdleTime))) /
                                         (1.0 * nCycles * (CpvAccess(kFactor) + 1));
...
CmiHandler startOperationHandlerFunc(char* msg) {
  ...
  if ((CmiMyPe() < CmiNumPes() / 2) || CpvAccess(twoway)) startOperation();
  return 0;
}
...
CmiStartFn mymain(int argc, char** argv) {
  ...
  if (argc > 2) CpvAccess(kFactor) = atoi(argv[2]);
