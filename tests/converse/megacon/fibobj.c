...
void CpmPack_fibobj_chare(fibobj_chare v) { return; }

void CpmUnpack_fibobj_chare(fibobj_chare v) { return; }
...
void fibobj_init() {
  fibobj_chare c = (fibobj_chare)malloc(sizeof(struct fibobj_chare_s));
  c->ppe = -1, c->ppos = (fibobj_chare)FIB_INPUT;
  c->total = 0;
  c->count = 1;
  Cpm_fibobj_calc(CpmLDB(), FIB_INPUT, CmiMyPe(), c);
}

void fibobj_moduleinit() { CpmInitializeThisModule(); }
