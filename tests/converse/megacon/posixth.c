...
void* posixth_top(void* x) {
  ...
  if (CrnRand() & 1) CthYield();
  if (CpvAccess(fibs) == 0) errck(Cpthread_cond_signal(&CpvAccess(donecond)));
  if (CrnRand() & 1) CthYield();
  return NULL;
}

void posixth_main(int argc, char** argv) {
  Cpthread_mutex_t dummymutex;
  int i;
  Cpthread_t t;
...
void posixth_init(void) { Cpthread_start_main(posixth_main, 0, 0); }
