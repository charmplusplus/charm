...
typedef struct { int me; } multisend_info;
...
void multisend_fail() { CmiAbort("data corrupted in multisend.\n"); }

static void checkMsg(multisendmsg* msg) {
  int i;
  for (i = 0; i < 10 - msg->me; i++) {
    if (msg->data[i] != (double)(i + msg->me)) multisend_fail();
  }
}

void multisend_handler(multisendmsg* msg, multisend_info* info) {
  if (msg->me != info->me) multisend_fail();
  checkMsg(msg);
...
void multisend_done_handler(multisendmsg* msg) {
  ...
  if (CpvAccess(multisend_replies) == nMulti) Cpm_megacon_ack(CpmSend(0));
}
...
#endif
  {
    ...
    for (m = 0; m < nMulti; m++) {
      ...
      if (m == 0)
        ...
        msg = (multisendmsg*)CmiAlloc(sizeof(multisendmsg));
...
void multisend_moduleinit() {
  ...
  for (m = 0; m < nMulti; m++) {
    ...
    i->me = m;
    CpvAccess(multisend_index)[m] =
        CmiRegisterHandlerEx((CmiHandlerEx)multisend_handler, i);
  }
