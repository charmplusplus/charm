...
typedef struct specmsg_chare_s { int next; } * specmsg_chare;
...
void specmsg_init() {
  int pe;
  for (pe = 1; pe < CmiNumPes(); pe++) Cpm_specmsg_begin(CpmSend(pe));
  specmsg_begin();
}

void specmsg_moduleinit() { CpmInitializeThisModule(); }
