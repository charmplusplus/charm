...
int TestInstance::<ompts:testcode:functionname>omp_for_schedule_static_3</ompts:testcode:functionname> ()
{
 // int threads;
  int i,lasttid;
  <ompts:orphan:vars>
...
/*  if (threads < 2)
  {
    ...
    CkPrintf("This test only works with at least two threads");
    return 0;
  }*/
//  else 
  {
    CkPrintf("Using an internal count of %d\nUsing a specified chunksize of %d\n", CFSMAX_SIZE, chunk_size);
...
#pragma omp flush(maxiter,notout)
	...
	/* If the chunk size was wrong, check if the end was reached */
	  ...
	  if (tids[i] == -1) {
	    if (i == CFSMAX_SIZE) {
	      CkPrintf( "Last thread had chunk size %d\n", tmp_count);
	      break;
	    }
	    else {
	      CkPrintf( "ERROR: Last thread (thread with number -1) was found before the end.\n");
	      result = 0;
	    }
	  }
	  else {
	    CkPrintf( "ERROR: chunk size was %d. (assigned was %d)\n", tmp_count, chunk_size);
	    result = 0;
	  }
	}
      }
      else {
	CkPrintf( "ERROR: Found thread with number %d (should be inbetween 0 and %d).", tids[i], threads - 1);
	result = 0;
      }
#ifdef VERBOSE
      CkPrintf( "%d: %d \n", i, tids[i]);
...
#pragma omp for <ompts:check>schedule(static)</ompts:check> nowait
  ...
  for (i = 0; i < LOOPCOUNT; i++)
      if (tids[i] != tids2[i]) {
          CkPrintf( "Chunk no. %d was assigned once to thread %d and later to thread %d.\n", i, tids[i],tids2[i]);
          result = 0;
      }
