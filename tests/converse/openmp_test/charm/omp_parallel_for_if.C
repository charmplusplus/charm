...
int TestInstance::<ompts:testcode:functionname>omp_parallel_for_if</ompts:testcode:functionname>(){
    int known_sum;
    <ompts:orphan:vars>
    int num_threads;
    int sum, sum2;
...
#pragma omp parallel for private(i) <ompts:check>if (control==1)</ompts:check>
    ...
    CkPrintf( "Number of threads determined by omp_get_num_threads: %d\n", num_threads);
