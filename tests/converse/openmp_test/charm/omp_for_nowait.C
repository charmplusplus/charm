...
int TestInstance::<ompts:testcode:functionname>omp_for_nowait</ompts:testcode:functionname> ()
{
	<ompts:orphan:vars>
		int result;
		int count;
...
#pragma omp for <ompts:check>nowait</ompts:check> 
		for (i = 0; i < LOOPCOUNT; i++) {
			if (i == 0) {
				CkPrintf( "Thread nr %d entering for loop and going to sleep.\n", rank);
				my_sleep(SLEEPTIME);
				count = 1;
#pragma omp flush(count)
				CkPrintf( "Thread nr %d woke up and set count = 1.\n", rank);
			}
		}
		
		CkPrintf( "Thread nr %d exited first for loop and enters the second.\n", rank);
#pragma omp for
		for (i = 0; i < LOOPCOUNT; i++) 
		{
#pragma omp flush(count)
