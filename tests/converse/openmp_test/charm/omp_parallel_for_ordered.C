...
int TestInstance::<ompts:testcode:functionname>omp_parallel_for_ordered</ompts:testcode:functionname>(){
	<ompts:orphan:vars>
	int sum;
	int is_larger;
	</ompts:orphan:vars>
  ...
  int ii;	
	...
	is_larger = 1;
	last_i = 0;
#pragma omp parallel for schedule(static,1) private(i, ii) <ompts:check>ordered</ompts:check>
	for (i = 1; i < 100; i++)
	{
		ii = i;
	<ompts:orphan>
<ompts:check>#pragma omp ordered</ompts:check><ompts:crosscheck></ompts:crosscheck>
	...
	known_sum = (99 * 100) / 2;
	CkPrintf(" known_sum = %d , sum = %d \n", known_sum, sum);
	CkPrintf(" is_larger = %d\n", is_larger);
