...
int TestInstance::<ompts:testcode:functionname>omp_for_schedule_guided</ompts:testcode:functionname> ()
{
    <ompts:orphan:vars>
	int * tids;
	int * chunksizes;
...
/*	if (threads < 2) {
	  ...
	  CkPrintf( "This test only works with at least two threads. Available were only %d thread(s).\n", threads);
	  return (0);
	}*/ /* end if */
 ...
 *******************************************************/
	{
	   ...
	   */
	  /*CkPrintf("# global_chunknr thread local_chunknr chunksize\n"); */
...
#ifdef VERBOSE
	  CkPrintf( "found\texpected\tconstant\n");
#endif
	  ...
	  for (i = 0; i < global_chunknr; i++)
	  {
		...
		if (abs (chunksizes[i] - expected_chunk_size) >= 2) {
		  result = 0;
#ifndef VERBOSE
		  CkPrintf( "Chunksize differed from expected value: %d instead of %d\n", chunksizes[i], expected_chunk_size);
		  return 0;
#endif
		} /* end if */
		...
		if (expected_chunk_size - chunksizes[i] < 0 )
		  CkPrintf( "Chunksize did not decrease: %d instead of %d\n", chunksizes[i],expected_chunk_size);
#endif
