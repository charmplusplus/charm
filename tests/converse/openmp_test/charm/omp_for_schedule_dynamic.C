...
int TestInstance::<ompts:testcode:functionname>omp_for_schedule_dynamic</ompts:testcode:functionname> ()
{
  int tid;
<ompts:orphan:vars>  
  int *tids;
...
/*it is possible for 2 adjacent chunks assigned to a same thread*/
	{
         result++;
  CkPrintf("The intermediate dispatch has wrong chunksize.\n");
  ...
  if ((tmp[count]%chunk_size)!=(CFDMAX_SIZE%chunk_size))
   { 
   result++;
  CkPrintf("the last dispatch has wrong chunksize.\n");
   }
