...
# run the test
    if ($opt_lang eq 'charm') {
      print "charm";
      print "$env_set_threads_command\n";
      $cmd ="$env_set_threads_command ./charmrun +p$numthreads ./$bin_name > $bin_name.out";
      print "cmd: $cmd\n";
    }
    else {
    ...
    if ($opt_lang eq 'charm') {
      timed_sys_command($cmd); 
      $exit_result = qx(grep "Result:"  $bin_name.out);
      if ($exit_result eq "") {
        $exit_status = 100; #100% fail
      }
      else {
        $exit_status = int((split /:/, $exit_result)[1]);
      }
