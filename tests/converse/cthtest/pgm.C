...
class threadStackChecker {
	...
	static int fn(char id,int cnt) {
		return ((int)id)+(cnt<<16);
	}
public:
	threadStackChecker(char id)
		:val(fn(id,-1)),count(-1) {}
...
void runThread(void *msg) 
{
  ...
  VERBOSE(
    char buf[NITER+2];
    for (int i=0;i<NITER+1;i++) buf[i]=' ';
    buf[0]=myId; 
    buf[1+iter]='0'+(iter%10);
    buf[NITER+1]=0;
    printf("%s\n",buf);
  )
    ...
    if (iter==(NITER/4) && myId<('A'+NSPAWN-1)) {
      CthThread nTh=CthCreateMigratable((CthVoidFn)runThread, 0, 16000);
      CthAwaken(nTh);
      CthYield();
    }
