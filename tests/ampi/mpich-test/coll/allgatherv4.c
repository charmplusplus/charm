...
{
    int i, j;
    int comm_size, comm_rank, comm_size_minus_one;
    double start, end;
    ...
    comm_size_minus_one = (comm_size == 1) ? 1 :comm_size - 1;
    displs[0] = 0;
    for (i = 0; i < comm_size; i++) {
        if (test_type == REGULAR)
            ...
            recvcounts[i] = (!i) ? (msg_size / 2) : (msg_size / (2 * comm_size_minus_one));
        else if (test_type == HALF_FULL)
            recvcounts[i] = (i < (comm_size / 2)) ? (2 * msg_size) : 0;
        else if (test_type == LINEAR_DECREASE) {
            tmp = 2 * msg_size * (comm_size_minus_one - i) / comm_size_minus_one;
            if (tmp != (int) tmp) {
