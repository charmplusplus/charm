...
//
// Current behaviour (Oct 20, 2022):
//
//  - Test hangs on more than one PE for charm v7.0.0
//
//  - Test fails on latest because of the dynamic_insertion error before getting
//    to the point v7.0.0 hangs
//
//  - It appears that when one of the initial elements is destroyed and then
//    recreated in a later iteration on a PE other than PE 0, it never receives
//    the ping.  (Could this be related to the load balancing issue (3605) that
//    was addressed by charm++ PR 3614?)
//
#include "dynamic_array_hang.h"
...
void Main::add_proc_to_map(const ElementId_t& id, const int proc) {
  if(iteration == 1) {
    ...
    if (refinement_level(id) == initial_refinement_level and
...
void DgElement::delete_old_elements(const size_t iteration) {
  ...
  if (iteration % 2 == 1 and lev == initial_refinement_level) {
    thisProxy[thisIndex].ckDestroy();
  } else if (iteration % 2 == 0 and lev == initial_refinement_level + 1) {
    thisProxy[thisIndex].ckDestroy();
  }
}
...
#include "dynamic_array_hang.def.h"
