...
\begin{itemize}
\item{Mark an entry method as \code{[aggregate]} in the \code{.ci} file:}
\begin{alltt}
entry [aggregate] void foo(Data d);
\end{alltt}
Entry methods that use the \code{[aggregate]} tag must have exactly one argument of a
fixed size.
\item{Include \code{NDMeshStreamer.h} before the inclusion of the
\code{.decl.h} file:}
\begin{alltt}
#include "NDMeshStreamer.h"
#include "SomeModule.decl.h"
\end{alltt}
\item{Add \code{-module NDMeshStreamer} to the link line when compiling}
\end{itemize}
