...
\begin{alltt}
 liveVizPollDeposit (this,req,startx,starty,sizex,sizey,img,max\_image\_data);
\end{alltt}

Valid combiners include: sum\_float\_image\_data, max\_float\_image\_data, sum\_image\_data, 
or max\_image\_data.
There is an optional and currently unused parameter ``timestep'' which may appear in older 
applications that use Poll Mode. LiveViz is backward compatible with these older applications.
