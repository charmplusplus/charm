...
A separate \kw{driver} subroutine runs for each block, and does the main work of the program.  Because there may be several blocks per processor, several \kw{driver} routines may be executing as threads simultaneously.  
...
\function{subroutine MBLK\_Wait\_update()}
     \kw{MBLK\_Wait\_update} call is a blocking call and is used in association 
...
\begin{itemize}
	\item \kw{bcnum} The boundary condition number to be associated with the 
	function.
	\item \kw{ghostWidth} The width of the ghost cells where this boundary
...
\function{subroutine MBLK\_Apply\_bc(bcnum, param1,param2,err)}
  ...
  \args{integer,intent(out)::err}
\kw{MBLK\_Apply\_bc} call is made to apply all boundary condition functions
