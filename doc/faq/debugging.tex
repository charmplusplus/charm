...
\subsection{How can I debug Charm++ programs?}
...
\subsection{How do I use charmdebug?}

Currently charmdebug is tested to work only under netlrts- non-SMP versions. With other versions,
...
\subsection{Can I use distributed debuggers like Allinea DDT and RogueWave TotalView?}
...
\subsection{How do I use {\em gdb} with Charm++ programs?}

It depends on the machine. On the netlrts- versions of Charm++, like netlrts-linux-x86\_64,
...
\subsection{When I try to use the {\em ++debug} option I get: {\tt remote
...
If you are working from a Windows machine, you need an X-win
...
\subsection{My debugging printouts seem to be out of order. How can I prevent this?}
...
\subsection{Is there a way to flush the print buffers in Charm++ (like
{\tt fflush()})?}
...
\subsection{My Charm++ program is causing a seg fault, and the debugger shows that
...
\subsection{Everything works fine on one processor, but when I run on
...
\subsection{I get the error: "{\tt Group ID is zero-{}- invalid!}". What does
this mean?}

The {\em group} it is referring to is the chare group. This
...
\subsection{I get the error: {\tt Null-Method Called. Program may have Unregistered
...
\subsection{When I run my program, it gives this error:}
...
\subsection{When I run my program, sometimes I get a {\tt Hangup}, and
