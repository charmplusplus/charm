...
of \uw{HelloMain}. The wait method calls CkExit() after all group members have called it (\em{ie} they have finished printing ``Hello World...''), and the \charmpp program exits.

\subsubsection{Functions of the ``decl.h'' and ``def.h'' files}

	The {\em decl.h} file provides decelerations for the proxy classes, of the concurrent objects declared in the ``.ci'' file. So the \uw{Hello.decl.h} file will have the deceleration of the class CProxy_HelloMain. Similarly it will also have the deceleration for the HelloGroup class. 
