...
computation begins, or to declare a regular  \CC\ subroutine \kw{initproc}
...
and \uw{bar::barProcInit} on every processor before the main computation starts.
Initnode calls are always executed before initproc calls.

Note that these routines should only do registration, not computation--
use a mainchare instead, which gets executed on only one processor,
