...
For arrays and vectors of builtin arithmetic types like ``int" and ``double", or
of types declared as ``PUPbytes'', \kw{PUParray} uses an even faster block
transfer, with one virtual function call per array or vector.
...
that takes an argument of type \verb+PUP::reconstruct+ for this
purpose. The runtime system code will call a \verb+PUP::reconstruct+
constructor in preference to a default constructor when it's
available. Where necessary, constructors
taking \verb+PUP::reconstruct+ should call the constructors of members
variables with \verb+PUP::reconstruct+ if applicable to that member.
