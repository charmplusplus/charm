...
Your \kw{PUP::er} will be called in one of the modes (i.e. packing,
...
which you expect the \kw{PUP::er} to be used.  Note that packing and
...
\kw{PUP::er} methods, are handled automatically for you in the ``p|a''
...
\kw{p.isPacking()} and
\kw{p.isUnpacking()} where you perform the appropriate allocation and deallocation.  See Section~\ref{sec:pupdynalloc} for more
information and examples.   

\paragraph{PUP operator |}

The easiest syntax to use for PUPs is the style developed for
parameter marshalling ``p|a'' where p is the name of the \kw{PUP::er}
and a is the object to be PUPed.  This is equivalent to ``p(a)'' in
most cases.  It works wherever a is a simple type, or a is PUP::able
(\ref{sec:pup::able}), or a has a \kw{PUP::er} defined, and \kw{operator |}
is not overridden for a.  

If you know that all your data fields can be handled by a simple byte
to byte copy then
\kw{\#define~CK\_DEFAULT\_BITWISE\_PUP=1} will default  \kw{operator |} for all user defined types to a bitwise copy operation.  We do not recommended it.  It creates a potential for difficult to trace errors to occur when new types are added which are not correctly handled bitwise and the programmer neglects to write a proper PUP.
