...
Your object's \uw{pup} method must save and restore all your object's data.
...
or equivalently, ``to call the pup method of''.
...
Pupping as bytes will prevent your pup method from 
...
Please note that if your object contains Structured Dagger code (see section~\ref{sec:sdag}) you must call the generated method \kw{\_\_sdag\_pup} to correctly pup the Structured Dagger state:
...
See the \index{PUP} section ``PUP'' for more details on pup methods
and the \kw{PUP::er} type.

The system uses one pup method to do both packing and unpacking by
