...
other processors}. In this mode, load balancer works in the 
background and application doesn't need to stop waiting for load 
...
application with multiple iterations, load balancing best happens after 
...
on one processor}, and starts load balancing phases from there. The 
application would take a break and wait for the load balancing and 
migration complete until it regains 
the control when load balancer calls back {\em ResumeFromSync()}, which is a 
virtual function of array element that must be defined in this mode.
