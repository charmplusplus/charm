...
descriptions:
\begin{itemize}
\item \kw{RefineLB}:     Move objects away from the most overloaded processors to reach average;
...
\item \kw{RecBisectBfLB}:        Recursively partition with Breadth first enumeration;
\item \kw{MetisLB}:      Use Metis(tm) to partitioning object communication graph;
\item \kw{CommLB}:       Greedy algorithm which takes communication graph into account;
\item \kw{Comm1LB}:      another variation of CommLB;
\item \kw{HeapCentLB}:   Similar to RefineLB, but using heap to sort by load;
\item \kw{GreedyRefLB}:  Apply greedy, then refine;
\item \kw{NeighborLB}:   A neighborhood load balancer
\end{itemize}
