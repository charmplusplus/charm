...
\subsection{Load Balancing Chares - seed load balancers}
\label{seedlb}
...
chares~\footnote{as opposed to chare arrays which are not balanced by seed load balancers}
 at creation time. 
When the chare is created on a processor, there is no movement of chares
due to the seed load balancer. 

Several variations of strategies have been being designed and analyzed. 
\begin{enumerate}
\item {\em random}\\  
...
\item {\em spray}\\  
 ...
 results in communication amongst neighborhood processors (parent and 
children processors) to compute global average load via periodic reduction. 
 It uses averaging of loads to determine how seeds should be
distributed.
\item {\em graph}\\  
 a strategy which imposes a dense graph organization on the processors,
 load exchange happens to neighbors only. The overloaded processors
 initiate the load balancing, where a processor send work to elsewhere
...
{\bf -balance foo} links with a seed load balancer module. 
All seed laod balaner modules should be compiled in the form of library
under charm/lib, named as libcldb-foo.a, where foo is the strategy 
name also used in the link time option.
