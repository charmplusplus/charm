...
\paragraph{\bf Array Section Reduction:} 
...
called ''CkSectionCookie'' is created by CkMulticastMgr for each 
array section that the array element belongs to.
When doing section reduction, the array element needs to pass the 
\kw{CkSectionCookie} as a parameter in the \kw{contribute()}. 
The \kw{CkSectionCookie} can be retrieved
from a message in a multicast entry function for that array section
using function call \kw{CkGetSectionCookie}:
...
Note that the cookie cannot be used as a local variable in the function,
the same cookie is needed for the next contribute. This is because cookie 
includes some context sensive information for example the reduction counter.
Function \kw{CkGetSectionCookie()} only update some part of the data in cookie.
...
\kw{setReductionClient} call to the section root member. A 
{\bf CkMcastReductionMsg} message will be passed to this callback, which 
...
\paragraph{\bf Array section multicast/reduction when migration happens:}
Using multicast/reduction, you don't need to worry about array migrations.
When migration happens, you can still use the \kw{CkSectionCookie} for
...
internally in \kw{CkMulticastMgr}. A new updated \kw{CkSectionCookie} is always
contained in a multicast message, so it is recommended that 
\kw{CkGetSectionCookie()} function is called everytime time when a multicast 
