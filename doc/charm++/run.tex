...
\item[{\tt ++scalable-start}]   Scalable start, or SMP-aware startup. It is useful for scalable process launch on multu-core systems since it creates only one ssh session per node and spawn all clients from the same ssh session                                                       
