...
| 7 | ~~Re-pin `AUTOFETCH_RECONVERSE_TAG` to reconverse **main**~~ | DONE, and re-done as reconverse moved: `f3f4110` (#3949) -> `0673ee8` (#3956, picks up reconverse#211 `CmiNodeRankOnPhysicalNode`) -> `2c50813e7` (#3959, picks up reconverse#212, the `CmiCheckAffinity` startup hang). Then `58921e9` (#3974), `f3ce4fd9b` (#3986), `7a47d5c` (#3998), and `b30ad31` = reconverse main 2026-09-24 (this bump: reconverse#252 list-send fan-out no longer shares a buffer between ranks -- the NAMD hybrid-LB / megatest listsend race; #256 mempools live until the network is torn down -- the exit-time SIGSEGV; #254 `decRef` acq_rel; #251 spantree on by default; #253 `+backend_poll_freq` default 4 -> 64; #250, #255, #150, #249, #245, #244). Standing obligation: every reconverse merge we depend on needs a pin bump. The pin is no longer a SHA in a CMake file: reconverse is the `contrib/reconverse` submodule and its gitlink is the pin, so a bump is `git -C contrib/reconverse checkout <sha>` plus committing the moved gitlink. That also retires the stale-build-directory hazard this row used to carry -- there is no CMake cache holding a previous SHA, because the source tree on disk is the pin |
