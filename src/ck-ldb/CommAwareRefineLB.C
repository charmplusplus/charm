...
class CommAwareRefineLB::ObjPeCommInfo {
  public:
    ...
    int obj_id;
    vector<CommAwareRefineLB::PeCommInfo> pcomm;
};

class CommAwareRefineLB::ProcCommGreater {
  public:
    bool operator()(CommAwareRefineLB::PeCommInfo p1, CommAwareRefineLB::PeCommInfo p2) {
...
void CommAwareRefineLB::work(LDStats* stats) {
  ...
  std::make_heap(parr_above_avg.begin(), parr_above_avg.end(),
      CommAwareRefineLB::ProcLoadGreater(parr));
...
inline void getPossiblePes(vector<int>& possible_pes, int vert,
  ...
  int index;
  int i, j, nbrid;
  CommAwareRefineLB::ObjPeCommInfo objpcomm;
 ...
 //   ogr->vertices[vert].recvFromList.size());
  ...
  for (i = 0; i < ogr->vertices[vert].sendToList.size(); i++) {
    ...
    if (tmp_map_pid_index.count(j) == 0) {
      ...
      CommAwareRefineLB::PeCommInfo pecomminf(j);
