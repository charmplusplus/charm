...
{
  static int message_type_number = -1;
...
be_operation::be_operation(AST_Type *rt, AST_Operation::Flags fl,
	    ...
	    : AST_Operation(rt, fl, n, p),
	      ...
	      p_marshallMessageNum(-1),
	      p_returnMarshallMessageNum(-1)
{
}
...
be_operation::hasOutOrInoutParameter(void)
{
  ...
  while (!(i.is_done())) {
    ...
    assert(d->node_type() == AST_Decl::NT_argument);
...
be_operation::hasNonVoidRetVal(void)
{
  ...
  if (return_type()->node_type() == AST_Decl::NT_pre_defined) {
    be_predefined_type *opRetType = 
      be_predefined_type::narrow_from_decl(return_type());
    return (opRetType->pt() != AST_PredefinedType::PT_void);
  } else {
    cerr << "Error: Unexpected return value in input file\n";
    return FALSE;
  }
}
