...
public:
  ...
  ck::BaseID array_element_id(void);
...
{
public:
  CkArrayElementMigrateMessage(CkArrayIndex idx_, const ck::BaseID& id_, bool ignoreArrival_,
...
private:
  // Map of ID to PE
  using LocationMap = std::unordered_map<ck::BaseID, CkLocEntry>;
  LocationMap locMap;

  using Listener = std::function<void(const ck::BaseID&, int)>;
...
public:
  ...
  void requestLocation(const ck::BaseID& id);

  // Entry methods for updating location tables across PEs
  void requestLocation(const ck::BaseID& id, int peToTell);
  void updateLocation(const CkLocEntry& e);

  // Update the location table when an element migrates away
  void recordEmigration(const ck::BaseID& id, int pe);

  // Query the local location table
  const CkLocEntry& getLocationEntry(const ck::BaseID& id) const
  {
    LocationMap::const_iterator itr = locMap.find(id);
    return itr != locMap.end() ? itr->second : CkLocEntry::nullEntry;
  }
  int getPe(const ck::BaseID& id) const { return getLocationEntry(id).pe; }
  int getEpoch(const ck::BaseID& id) const { return getLocationEntry(id).epoch; }
  int homePe(const ck::BaseID& id) const { return id.getHome(); }

  // Insertion and removal
  void insert(const ck::BaseID& id, int epoch = 0);
  void erase(const ck::BaseID& id) { locMap.erase(id); }

  void addListener(Listener l) { listeners.push_back(l); }
  void notifyListeners(const ck::BaseID& id, int pe)
  {
...
private:
  ...
  using MsgBuffer = std::unordered_map<ck::BaseID, std::vector<CkArrayMessage*> >;
  ...
  using IdxIdMap = std::unordered_map<CkArrayIndex, ck::BaseID, IndexHasher>;
  using LocRecHash = std::unordered_map<ck::BaseID, CkLocRec*>;
  using ElemMap = std::unordered_map<ck::BaseID, CkMigratable*>;

  // Internal interface:
  void AtSyncBarrierReached();
  // Add given element array record at idx, replacing the existing record
  void insertRec(CkLocRec* rec, const ck::BaseID& id);

  // Remove this entry from the table (does not delete record)
  void removeFromTable(const ck::BaseID& id);
  ...
  inline void insertID(const CkArrayIndex& idx, const ck::BaseID& id)
  {
    if (compressor)
      return;
    idx2id[idx] = id;
...
public:
  ...
  CmiUInt8 idCounter;
  ck::BaseID getNewObjectID(const CkArrayIndex& idx);
  ...
  void deliverAnyBufferedMsgs(const ck::BaseID&, MsgBuffer& buffer);
  void deliverAnyBufferedMsgs(const CkArrayIndex&, const ck::BaseID&, IndexMsgBuffer&);
  ...
  int homePe(const ck::BaseID& id) const
  {
    return id.getHome();
  }
  ...
  int whichPe(const ck::BaseID& id) const { return cache->getPe(id); }
  ...
  ck::BaseID lookupID(const CkArrayIndex& idx) const
  {
    if (compressor)
    {
  ...
  bool lookupID(const CkArrayIndex& idx, ck::BaseID& id) const
  {
    if (compressor)
    {
  ...
  CkArrayIndex lookupIdx(const ck::BaseID& id) const
  {
    if (compressor)
    {
      return compressor->decompress(id.getBits());
    }
    else
    {
  ...
  CkLocRec* elementNrec(const ck::BaseID& id);
  ...
  void restore(const CkArrayIndex& idx, const ck::BaseID& id, PUP::er& p);
  /// Insert and unpack this array element from this checkpoint (e.g., from
  /// CkLocation::pup)
  void resume(const CkArrayIndex& idx, const ck::BaseID& id, PUP::er& p, bool notify = true,
              bool = false);
  ...
  void deliverAnyBufferedRdmaMsgs(const ck::BaseID& id);
  ...
  // carrying out rgets)
  void processAfterActiveRgetsCompleted(const ck::BaseID& id);
  ...
  // type, int opts = 0);
  int deliverMsg(CkArrayMessage* m, CkArrayID mgr, const ck::BaseID& id, const CkArrayIndex* idx,
  ...
  void deliverAllBufferedMsgs(const ck::BaseID& id);
  void deliverAllBufferedMsgs(const CkArrayIndex&, const ck::BaseID&);
