...
public:
  ...
  virtual void populateInitial(int arrayHdl,CkArrayIndex& numElements,void *ctorMsg,CkArray *mgr);
...
//Interface used by external users:
	...
	int lastKnown(CmiUInt8 id);
        ...
        inline CmiUInt8 lookupID(const CkArrayIndex &idx) {
          if (compressor) {
        ...
        inline bool lookupID(const CkArrayIndex &idx, CmiUInt8& id) {
          if (compressor) {
...
//Interface used by array manager and proxies
	...
	void populateInitial(CkArrayIndex& numElements,void *initMsg,CkArray *mgr)
	...
	int deliverMsg(CkArrayMessage *m, CkArrayID mgr, CmiUInt8 id, CkDeliver_t type, int opts = 0);
