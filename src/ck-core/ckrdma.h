...
#if CMK_SMP
  int getNumPeers() const { return numPeers.load(std::memory_order_acquire); }
  void setNumPeers(int r) { return numPeers.store(r, std::memory_order_release); }
  int incNumPeers() { return numPeers.fetch_add(1, std::memory_order_release); }
  int decNumPeers() { return numPeers.fetch_sub(1, std::memory_order_release); }
#else
...
#if CMK_SMP
  int getNumElems() const { return numElems.load(std::memory_order_acquire); }
  void setNumElems(int r) { return numElems.store(r, std::memory_order_release); }
  int incNumElems() { return numElems.fetch_add(1, std::memory_order_release); }
  int incNumElems(int r) { return numElems.fetch_add(r, std::memory_order_release); }
  int decNumElems() { return numElems.fetch_sub(1, std::memory_order_release); }
#else
...
// for the destination or receiver buffers and some additional information required for
// processing and acknowledgment handling. The space for additional information is
// typically equal to sizeof(NcpyEmInfo) + numops * sizeof(NcpyEmBufferInfo)
...
struct NcpyEmInfo
{
  int numOps;   // number of zerocopy operations i.e number of buffers sent using
                // CkSendBuffer
  ...
  void* msg;  // pointer to the Charm++ message which will be enqueued after completion of
              // all Rgets
...
public:
  ...
  explicit CkNcpyBuffer(const void* ptr_, size_t cnt_,
  ...
  explicit CkNcpyBuffer(const void* ptr_, size_t cnt_, CkCallback& cb_,
                        unsigned short int regMode_ = CK_BUFFER_REG,
                        unsigned short int deregMode_ = CK_BUFFER_DEREG)
  {
  ...
  {
    CkPrintf(
        "[%d][%d][%d] CkNcpyBuffer print: ptr:%p, size:%zu, pe:%d, regMode=%d, "
        "deregMode=%d, ref:%p, refAckInfo:%p\n",
        CmiMyPe(), CmiMyNode(), CmiMyRank(), ptr, cnt, pe, regMode, deregMode, ref,
        refAckInfo);
  }

  void init(const void* ptr_, size_t cnt_, CkCallback& cb_,
            unsigned short int regMode_ = CK_BUFFER_REG,
            unsigned short int deregMode_ = CK_BUFFER_DEREG)
  {
    cb = cb_;
  ...
  friend void constructDestinationBufferObject(NcpyOperationInfo* info,
                                               CkNcpyBuffer& dest);

  friend envelope* CkRdmaIssueRgets(envelope* env, ncpyEmApiMode emMode,
                                    void* forwardMsg);
  friend void CkRdmaIssueRgets(envelope* env, void** arrPtrs, int* arrSizes,
                               int localIndex, CkNcpyBufferPost* postStructs);
  ...
  friend void performEmApiNcpyTransfer(CkNcpyBuffer& source, CkNcpyBuffer& dest,
                                       int opIndex, CmiSpanningTreeInfo* t,
                                       NcpyEmInfo* ref, int extraSize,
                                       CkNcpyMode ncpyMode, int rootNode,
                                       ncpyEmApiMode emMode);

  friend void performEmApiRget(CkNcpyBuffer& source, CkNcpyBuffer& dest, int opIndex,
                               NcpyEmInfo* ref, int extraSize, int rootNode,
                               ncpyEmApiMode emMode);

  friend void performEmApiCmaTransfer(CkNcpyBuffer& source, CkNcpyBuffer& dest,
                                      CmiSpanningTreeInfo* t, ncpyEmApiMode emMode);

  friend void performEmApiMemcpy(CkNcpyBuffer& source, CkNcpyBuffer& dest,
                                 ncpyEmApiMode emMode);
...
// Returns CkNcpyMode::CMA if both the PEs are in the same physical node and CMA can be
// used Returns CkNcpyMode::RDMA if RDMA needs to be used
 ...
 * ****************************/
static inline CkNcpyBuffer CkSendBuffer(const void* ptr_, CkCallback& cb_,
                                        unsigned short int regMode_ = CK_BUFFER_REG,
                                        unsigned short int deregMode_ = CK_BUFFER_DEREG)
{
...
static inline CkNcpyBuffer CkSendBuffer(const void* ptr_,
                                        unsigned short int regMode_ = CK_BUFFER_REG,
                                        unsigned short int deregMode_ = CK_BUFFER_DEREG)
{
...
void CkRdmaAsyncPostPreprocess(envelope* env, int numops, CkNcpyBufferPost* postStructs,
                               CmiUInt8 arrayIndex, void* ackInfo);

void CkRdmaAsyncPostPreprocess(envelope* env, int numops, CkNcpyBufferPost* post);

// This structure is used to store the buffer information specific to each buffer being
// sent using the Zerocopy Entry Method API. A variable of the structure stores the
// information associated with each buffer
struct NcpyEmBufferInfo
{
  int index;  // Represents the index of the buffer information (from 0,1... numops - 1)
  NcpyOperationInfo
      ncpyOpInfo;  // Stores all the information required for the zerocopy operation
};
...
void CkRdmaIssueRgets(envelope* env, void** arrPtrs, int* arrSizes, int localIndex,
...
// Structure is used for storing source buffer info to de-reg and invoke acks after
// completion of CMA operations
...
struct NcpyBcastAckInfo
{
  ...
  // worker thread (in the case of a memcpy transfer) and the comm thread (CMA or RDMA
  // transfer) can increment the variable.
...
#if CMK_SMP
  int getCounter() const { return counter.load(std::memory_order_acquire); }
  void setCounter(int r) { return counter.store(r, std::memory_order_release); }
  int incCounter() { return counter.fetch_add(1, std::memory_order_release); }
  int decCounter() { return counter.fetch_sub(1, std::memory_order_release); }
#else
...
// Method called on the p2p source to store information for de-reg and ack handling for
// CMA transfers
void CkRdmaPrepareZCMsg(envelope* env, int node);

// Method called on ZC API source (internally calls CkRdmaPrepareBcastMsg or
// CkRdmaPrepareZCMsg)
void CkRdmaPrepareP2PMsg(envelope* env);

void CkReplaceSourcePtrsInBcastMsg(envelope* env, NcpyBcastInterimAckInfo* bcastAckInfo,
                                   int origPe);

// Method called to extract the parent bcastAckInfo from the received message for ack
// handling
const void* getParentBcastAckInfo(void* msg, int& srcPe);

// Allocate a NcpyBcastInterimAckInfo and return the pointer
NcpyBcastInterimAckInfo* allocateInterimNodeAckObj(envelope* myEnv, envelope* myChildEnv,
                                                   int pe);
...
void handleMsgUsingCMAPostCompletionForSendBcast(envelope* copyenv, envelope* env,
                                                 CkNcpyBuffer& source);

void processBcastSendEmApiCompletion(NcpyEmInfo* ncpyEmInfo, int destPe);

// Method called on intermediate nodes after RGET to switch old source pointers with my
// pointers
void CkReplaceSourcePtrsInBcastMsg(envelope* prevEnv, envelope* env, void* bcastAckInfo,
                                   int origPe);

void processBcastRecvEmApiCompletion(NcpyEmInfo* ncpyEmInfo, int destPe);

// Method called on the root node and other intermediate parent nodes on completion of
// RGET through ZC Bcast
...
void CmiForwardProcBcastMsg(int size,
                            char* msg);  // for forwarding proc messages to my child nodes
void CmiForwardNodeBcastMsg(
    int size, char* msg);  // for forwarding node queue messages to my child nodes
...
// Handler method invoked on the ZC p2p API source for de-registration and ack handling
// for CMA transfers
...
void setNcpyEmInfo(NcpyEmInfo* ref, envelope* env, int& numops, void* forwardMsg,
                   ncpyEmApiMode emMode);
...
void setPosted(std::vector<int>* tagArray, envelope* env, CmiUInt8 elemIndex, int numops,
               int opIndex);

bool isUnposted(std::vector<int>* tagArray, envelope* env, CmiUInt8 elemIndex,
                int numops);

void* extractStoredBuffer(std::vector<int>* tagArray, envelope* env, CmiUInt8 elemIndex,
                          int numops, int opIndex);

int CkPerformRget(CkNcpyBufferPost& post, void* destBuffer, int destSize);

void setPostStruct(CkNcpyBufferPost* ncpyPost, int index, CkNcpyBuffer& buffObj,
                   CmiUInt8 elemIndex);
...
{
  int destSize =
      (std::is_same<T, void>::value) ? size : sizeofutils::safe_sizeof(buffer) * size;
