...
#define CKEXIT_FUNC_RECOMPOSER(argsWithParentheses) \
  CKEXIT_FUNC_CHOOSER argsWithParentheses
#define CKEXIT_CHOOSE_FROM_ARG_COUNT(...) \
  CKEXIT_FUNC_RECOMPOSER((__VA_ARGS__, CKEXIT_2, CKEXIT_1, ))
#define CKEXIT_NO_ARG_EXPANDER() , , CKEXIT_0
#define CKEXIT_MACRO_CHOOSER(...) \
  CKEXIT_CHOOSE_FROM_ARG_COUNT(CKEXIT_NO_ARG_EXPANDER __VA_ARGS__())
...
#  endif
  extern void* CkAllocMsg(int msgIdx, int msgBytes, int prioBits,
                          GroupDepNum groupDepNum = GroupDepNum{});
#endif
 ...
 *****************************************************************************/
  ...
  extern void registerChareMsgRecvExtCallback(void (*cb)(int, void*, int, int, char*,
                                                         int));
  extern void registerGroupMsgRecvExtCallback(void (*cb)(int, int, int, char*, int));
  extern void registerArrayMsgRecvExtCallback(void (*cb)(int, int, int*, int, int, char*,
                                                         int));
  extern void registerArrayBcastRecvExtCallback(void (*cb)(int, int, int, int, int*, int,
                                                           int, char*, int));
  extern void registerArrayElemLeaveExtCallback(int (*cb)(int, int, int*, char**, int));
  extern void registerArrayElemJoinExtCallback(void (*cb)(int, int, int*, int, char*,
                                                          int));
  ...
  extern void StartCharmExt(
      int argc,
      char** argv);  // start Converse/Charm, argv are the command-line arguments
  ...
  extern void CkStartQDExt_ArrayCallback(int aid, int* idx, int ndims, int epIdx,
                                         int fid);
  extern void CkStartQDExt_SectionCallback(int sid_pe, int sid_cnt, int rootPE, int ep);
  extern void registerCreateCallbackMsgExtCallback(void (*cb)(void*, int, int, int, int*,
                                                              char**, int*));
  ...
  */
  ...
  extern int CkRegisterMsg(const char* name, CkPackFnPtr pack, CkUnpackFnPtr unpack,
...
#define CK_EP_INLINE (1 << 8)
  ...
  extern int CkRegisterEp(const char* name, CkCallFnPtr call, int msgIdx, int chareIdx,
                          int ck_ep_flags);
...
#if CMK_CHARM4PY
  void CkRegisterMainChareExt(const char* s, const char** emNames, int emNamesStart,
                              int numEntryMethods, int* chareIdx, int* startEpIdx);
  extern void CkRegisterGroupExt(const char* s, const char** emNames, int emNamesStart,
                                 int numEntryMethods, int* chareIdx, int* startEpIdx);
  extern void CkRegisterGroupExt(const char* s, const char** emNames, int emNamesStart,
                                 int numEntryMethods, int* chareIdx, int* startEpIdx);
  extern void CkRegisterSectionManagerExt(const char* s, const char** emNames,
                                          int emNamesStart, int numEntryMethods,
                                          int* chareIdx, int* startEpIdx);
  extern void CkRegisterArrayMapExt(const char* s, const char** emNames, int emNamesStart,
                                    int numEntryMethods, int* chareIdx, int* startEpIdx);
  extern void CkRegisterArrayExt(const char* s, const char** emNames, int emNamesStart,
                                 int numEntryMethods, int* chareIdx, int* startEpIdx);
  extern void CkRegisterReadonlyExt(const char* name, const char* type, size_t msgSize,
                                    char* msg);
#endif
  ...
  extern void CkRegisterReadonly(const char* name, const char* type, size_t size,
                                 void* ptr, CkPupReadonlyFnPtr pup_fn);
  /** Register this readonly message.*/
  extern void CkRegisterReadonlyMsg(const char* name, const char* type, void** pMsg);
  ...
  */
  ...
  typedef struct _ckGroupID
  {
    ...
    inline int operator==(const struct _ckGroupID& gid) const { return (gid.idx == idx); }
    inline int operator<(const struct _ckGroupID& gid) const { return (gid.idx < idx); }
#endif
  } CkGroupID;
...
typedef struct envelope envelope;
#endif
  extern void CkCreateChare(int chareIdx, int constructorIdx, void* msg, CkChareID* vid,
                            int destPE);
  ...
  extern void CkCreateLocalNodeGroup(CkGroupID groupID, int constructorIdx,
                                     envelope* env);

#if CMK_CHARM4PY
  extern int CkCreateGroupExt(int cIdx, int eIdx, int num_bufs, char** bufs,
                              int* buf_sizes);
  extern int CkCreateArrayExt(int cIdx, int ndims, int* dims, int eIdx, int num_bufs,
                              char** bufs, int* buf_sizes, int map_gid, char useAtSync,
                              int otherIdx);
  extern void CkInsertArrayExt(int aid, int ndims, int* index, int epIdx, int onPE,
                               int num_bufs, char** bufs, int* buf_sizes, char useAtSync);
  ...
  typedef enum
  {
    ...
    ROPeerCompletionMsg =
        11,  // Message to signal completion of RO Data transfer using Zcpy API
             // ^(used by child nodes to signal completion to their parent node in the
             // bcast spanning tree)
    ROChildCompletionMsg =
        12,             // Message to signal completion of RO Data transfer using Zcpy API
    ...
    ArrayBcastFwdMsg = 25,  // A bcast which arrived on node and must be forwarded to
...
#  define CK_MSGOPTIONAL
  ...
  extern void CkSendMsgBranchInline(int eIdx, void* msg, int destPE, CkGroupID gID,
                                    int opts CK_MSGOPTIONAL);
  extern void CkSendMsgBranchMulti(int eIdx, void* msg, CkGroupID gID, int npes,
                                   const int* pes, int opts CK_MSGOPTIONAL);
  extern void CkSendMsgBranchGroup(int eIdx, void* msg, CkGroupID gID, CmiGroup grp,
                                   int opts CK_MSGOPTIONAL);
  extern void CkSendMsgNodeBranch(int eIdx, void* msg, int destNode, CkGroupID gID,
                                  int opts CK_MSGOPTIONAL);
  extern void CkSendMsgNodeBranchInline(int eIdx, void* msg, int destNode, CkGroupID gID,
                                        int opts CK_MSGOPTIONAL);
  extern void CkSendMsgNodeBranchMulti(int eIdx, void* msg, CkGroupID gID, int npes,
                                       const int* nodes, int opts CK_MSGOPTIONAL);
  extern void CkBroadcastWithinNode(int eIdx, void* msg, CkGroupID gID,
                                    int opts CK_MSGOPTIONAL);
  ...
  extern void CkChareExtSend_multi(int onPE, void* objPtr, int epIdx, int num_bufs,
  ...
  extern void CkGroupExtSend(int gid, int npes, const int* pes, int epIdx, char* msg,
                             int msgSize);
  /// Send msg to group copying data into CkMessage from multiple input buffers
  extern void CkGroupExtSend_multi(int gid, int npes, const int* pes, int epIdx,
                                   int num_bufs, char** bufs, int* buf_sizes);
  ...
  /// of dimensions of the index. If ndims <= 0, msg will be broadcasted to all array
  /// elements
  extern void CkArrayExtSend(int aid, int* idx, int ndims, int epIdx, char* msg,
                             int msgSize);
  /// Send msg to array copying data into CkMessage from multiple input buffers
  extern void CkArrayExtSend_multi(int aid, int* idx, int ndims, int epIdx, int num_bufs,
                                   char** bufs, int* buf_sizes);
#endif
