...
void CkCallback::send(void *msg) const
{
  ...
  case sendChare: //Send message to a chare
    ...
    if (_entryTable[d.chare.ep]->isInline)
      CkSendMsgInline(d.chare.ep, msg, &d.chare.id); //inline send-to-chare
    else
  ...
  case sendGroup: //Send message to a group element
    ...
    if (_entryTable[d.group.ep]->isInline)
  ...
  case sendArray: //Send message to an array element
    ...
    if (_entryTable[d.array.ep]->isInline)
  ...
  case bcastSection: {
      ...
      break;
    } // braces to create scope and avoid crosses initialization error
