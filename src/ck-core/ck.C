...
Chare::~Chare()
{
  ...
  */
  if (chareIdx != -1)
  {
    ...
    {
      CkChareID* pCid = (CkChareID*)_allocMsg(DeleteVidMsg, sizeof(CkChareID));
...
void Chare::pup(PUP::er& p)
{
  ...
  p(chareIdx);
  if (chareIdx != -1)
    thishandle.objPtr = (void*)(CmiIntPtr)chareIdx;
#endif
  p(ckInitialized);
#if CMK_ERROR_CHECKING
  p(magic);
...
int Chare::ckGetChareType() const { return -3; }
...
void Chare::ckDebugPup(PUP::er& p) { pup(p); }
...
IrrGroup::IrrGroup(void) { thisgroup = CkpvAccess(_currentGroup); }
...
char* IrrGroup::ckDebugChareName() { return strdup(_chareTable[ckGetChareType()]->name); }
...
void CkDelegateMgr::ChareSend(CkDelegateData* pd, int ep, void* m, const CkChareID* c,
                              int onPE)
{
  CkSendMsg(ep, m, c);
}
...
void CkDelegateMgr::GroupSectionSend(CkDelegateData* pd, int ep, void* m, int nsid,
                                     CkSectionID* s)
{
  CkSendMsgBranchMulti(ep, m, s->_cookie.get_aid(), s->pelist.size(), s->pelist.data());
}
void CkDelegateMgr::NodeGroupSend(CkDelegateData* pd, int ep, void* m, int onNode,
                                  CkNodeGroupID g)
{
  CkSendMsgNodeBranch(ep, m, onNode, g);
}
void CkDelegateMgr::NodeGroupBroadcast(CkDelegateData* pd, int ep, void* m,
                                       CkNodeGroupID g)
{
  CkBroadcastMsgNodeBranch(ep, m, g);
}
void CkDelegateMgr::NodeGroupSectionSend(CkDelegateData* pd, int ep, void* m, int nsid,
                                         CkSectionID* s)
{
  CkSendMsgNodeBranchMulti(ep, m, s->_cookie.get_aid(), s->pelist.size(),
                           s->pelist.data());
}
void CkDelegateMgr::ArrayCreate(CkDelegateData* pd, int ep, void* m,
                                const CkArrayIndex& idx, int onPE, CkArrayID a)
{
  CProxyElement_ArrayBase ap(a, idx);
  ap.ckInsert((CkArrayMessage*)m, ep, onPE);
}
void CkDelegateMgr::ArraySend(CkDelegateData* pd, int ep, void* m,
...
void CkDelegateMgr::ArraySectionSend(CkDelegateData* pd, int ep, void* m, int nsid,
                                     CkSectionID* s, int opts)
{
...
void CProxy::pup(PUP::er& p)
{
  ...
  if (!delegatedGroupId.isZero())
  {
    ...
    if (delegatedMgr == NULL)
    {
      ...
      delegatedPtr =
          static_cast<CkDelegateMgr*>(obj)->DelegatePointerPup(p, delegatedPtr);
      free(obj);
    }
    else
    {
...
#define CKSECTIONID_CONSTRUCTOR_DEF(index)                                            \
  CkSectionID::CkSectionID(const CkArrayID& aid, const CkArrayIndex##index* elems,    \
  ...
  CkSectionID::CkSectionID(const CkArrayID& aid,                                      \
                           const std::vector<CkArrayIndex##index>& elems, int factor) \
      : bfactor(factor)                                                               \
  {                                                                                   \
...
CkSectionID::CkSectionID(const CkGroupID& gid, const int* _pelist, const int _npes,
                         int factor)
    : bfactor(factor)
{
  _cookie.get_aid() = gid;
...
CkSectionID::CkSectionID(const CkGroupID& gid, const std::vector<int>& _pelist,
                         int factor)
...
void QdCreate(int n) { CpvAccess(_qd)->create(n); }

void QdProcess(int n) { CpvAccess(_qd)->process(n); }

void CkSetRefNum(void* msg, CMK_REFNUM_TYPE ref) { UsrToEnv(msg)->setRef(ref); }

CMK_REFNUM_TYPE CkGetRefNum(void* msg) { return UsrToEnv(msg)->getRef(); }

int CkGetSrcPe(void* msg) { return UsrToEnv(msg)->getSrcPe(); }

int CkGetSrcNode(void* msg) { return CmiNodeOf(CkGetSrcPe(msg)); }

void* CkLocalBranch(CkGroupID gID) { return _localBranch(gID); }
...
void* CkLocalBranchOther(CkGroupID gID, int rank) { return _localBranchOther(gID, rank); }
...
char** CkGetArgv(void) { return CkpvAccess(Ck_argv); }
int CkGetArgc(void) { return CmiGetArgc(CkpvAccess(Ck_argv)); }
...
inline void _pushObj(Chare* obj) { CkpvAccess(runningObjs).emplace_back(obj); }
...
#endif
  const auto msgtype =
      (msg == NULL) ? LAST_CK_ENVELOPE_TYPE : UsrToEnv(msg)->getMsgtype();
...
static inline void _invokeEntryNoTrace(int epIdx, envelope* env, void* obj)
{
  ...
  if (CMI_ZC_MSGTYPE(UsrToEnv(msg)) == CMK_ZC_P2P_RECV_MSG ||
    ...
    CkDeliverMessageReadonly(
        epIdx, msg,
        obj);  // Do not free a P2P_RECV_MSG or BCAST_RECV_MSG or a BCAST_RECV_DONE_MSG
  else
...
void CkCreateLocalGroup(CkGroupID groupID, int epIdx, envelope* env)
{
  ...
  _invokeEntryNoTrace(epIdx, env,
                      obj); /* can't trace groups: would cause nested begin's */
...
static CkGroupID _nodeGroupCreate(envelope* env)
{
  CkGroupID groupNum;
  CmiImmediateLock(
      CksvAccess(_nodeGroupTableImmLock));  // change for proc 0 and other processors
...
{
  int idx;
  int x = (int)ceil(log((double)numNodes) /
                    log((double)2));  // number of bits needed to store node number
  int n = 32 - (x + 1);               // number of bits remaining for the index
  idx = (myNode << n) + numGroups;  // add number of processors, shift by the no. of bits
                                    // needed, then add the next available index
  // of course this won't work when int is 8 bytes long on T3E
  // idx |= 0x80000000;                                      // set the most significant
  // bit to 1
  idx = -idx;
...
static void _processNewVChareMsg(CkCoreState* ck, envelope* env)
{
  ...
  int idx;
  auto* obj = _allocNewChare(env, idx);
  CkChareID* pCid = (CkChareID*)_allocMsg(FillVidMsg, sizeof(CkChareID));
  pCid->onPE = CkMyPe();
#ifndef CMK_CHARE_USE_PTR
  pCid->objPtr = (void*)(CmiIntPtr)idx;
#else
...
/**
 ...
 scheduled again.
*/
static inline IrrGroup* _lookupGroupAndBufferIfNotThere(const CkCoreState* ck,
                                                        const envelope* env,
                                                        const CkGroupID& groupID)
{
  ...
  if (obj == NULL)
  { /* groupmember not yet created: stash message */
...
IrrGroup* lookupGroupAndBufferIfNotThere(CkCoreState* ck, envelope* env,
                                         const CkGroupID& groupID)
{
  return _lookupGroupAndBufferIfNotThere(ck, env, groupID);
}

static inline void _deliverForBocMsg(CkCoreState* ck, int epIdx, envelope* env,
                                     IrrGroup* obj)
{
  _invokeEntry(epIdx, env, obj);
...
static inline void _deliverForNodeBocMsg(CkCoreState* ck, int epIdx, envelope* env,
                                         void* obj)
{
  env->setEpIdx(epIdx);
  _invokeEntry(epIdx, env, obj);
...
static void _processArrayEltMsg(CkCoreState* ck, envelope* env)
{
  ...
  if (iter != object_map.end())
  {
    ...
    if (iter2 != localLocMgr->bufferedActiveRgetMsgs.end())
    {
      ...
      iter2->second.push_back(
          msg);  // Buffer msg for now and handle it after rgets complete
      return;
    }
...
void _processHandler(void* converseMsg, CkCoreState* ck)
{
  ...
  if (CMI_ZC_MSGTYPE(env) == CMK_ZC_P2P_SEND_MSG ||
      CMI_ZC_MSGTYPE(env) == CMK_ZC_BCAST_SEND_MSG)
  {
    envelope* prevEnv = env;

    // Determine mode depending on the message
    ncpyEmApiMode mode = (CMI_ZC_MSGTYPE(env) == CMK_ZC_BCAST_SEND_MSG)
                             ? ncpyEmApiMode::BCAST_SEND
                             : ncpyEmApiMode::P2P_SEND;
...
// #endif
  ...
  switch (env->getMsgtype())
  {
    ...
    case BocInitMsg:  // Group creation message
      TELLMSGTYPE(
          CkPrintf("proc[%d]: _processHandler with msg type: BocInitMsg\n", CkMyPe());)
    ...
    case ArrayBcastFwdMsg:
      TELLMSGTYPE(
          CkPrintf("proc[%d]: _processHandler with msg type: ForBocMsg\n", CkMyPe());)
    ...
    case ForNodeBocMsg:  // Nodegroup entry method message (non creation)
      TELLMSGTYPE(
          CkPrintf("proc[%d]: _processHandler with msg type: ForNodeBocMsg\n", CkMyPe());)
    ...
    case NewChareMsg:  // Singleton chare creation message
      TELLMSGTYPE(
          CkPrintf("proc[%d]: _processHandler with msg type: NewChareMsg\n", CkMyPe());)
      if (env->isPacked())
        CkUnpackMessage(&env);
    ...
    case NewVChareMsg:  // Singleton virtual chare creation message
      TELLMSGTYPE(
          CkPrintf("proc[%d]: _processHandler with msg type: NewVChareMsg\n", CkMyPe());)
      if (env->isPacked())
        CkUnpackMessage(&env);
    ...
    case ForChareMsg:  // Singeton chare entry method message (non creation)
      TELLMSGTYPE(
          CkPrintf("proc[%d]: _processHandler with msg type: ForChareMsg\n", CkMyPe());)
      if (env->isPacked())
        CkUnpackMessage(&env);
    ...
    case ForVidMsg:  // Singleton virtual chare entry method message (non creation)
      TELLMSGTYPE(
          CkPrintf("proc[%d]: _processHandler with msg type: ForVidMsg\n", CkMyPe());)
      _processForVidMsg(ck, env);
      break;
    case FillVidMsg:  // Message sent back from the real chare PE to the virtual chare PE
                      // to fill the VidBlock (called when the real chare is constructed)
      TELLMSGTYPE(
          CkPrintf("proc[%d]: _processHandler with msg type: FillVidMsg\n", CkMyPe());)
      _processFillVidMsg(ck, env);
      break;
    case DeleteVidMsg:  // Message sent back from the real chare PE to the virtual chare
                        // PE to delete the Vidblock (called when the real chare is
                        // deleted by the destructor)
      TELLMSGTYPE(
          CkPrintf("proc[%d]: _processHandler with msg type: DeleteVidMsg\n", CkMyPe());)
...
void _infoFn(void* converseMsg, CldPackFn* pfn, int* len, int* queueing, int* priobits,
...
void _skipCldHandler(void* converseMsg)
{
  ...
  {
    CqsEnqueueGeneral((Queue)CpvAccess(CsdGridQueue), env, env->getQueueing(),
                      env->getPriobits(), (unsigned int*)env->getPrioPtr());
  }
  else
  {
    CqsEnqueueGeneral((Queue)CpvAccess(CsdSchedQueue), env, env->getQueueing(),
                      env->getPriobits(), (unsigned int*)env->getPrioPtr());
  }
#else
  CqsEnqueueGeneral((Queue)CpvAccess(CsdSchedQueue), env, env->getQueueing(),
...
static inline int _prepareMsg(int eIdx, void* msg, const CkChareID* pCid)
{
  ...
  {  // Virtual chare ID (VID)
    ...
    if (pe == CkMyPe())
    {
      ...
      {  // The vidblock is not ready-- forget it
        vblk->send(env);
        return -1;
      }
    }
    else
    {  // Valid vidblock for another PE:
...
static inline envelope* _prepareImmediateMsgBranch(int eIdx, void* msg, CkGroupID gID,
                                                   int type)
{
...
static inline void _sendMsgBranchMulti(int eIdx, void* msg, CkGroupID gID, int npes,
                                       const int* pes)
{
...
void CkSendMsgBranchMultiImmediate(int eIdx, void* msg, CkGroupID gID, int npes,
                                   const int* pes)
{
#if CMK_IMMEDIATE_MSG && !CMK_SMP
...
void CkSendMsgBranchMulti(int eIdx, void* msg, CkGroupID gID, int npes, const int* pes,
                          int opts)
{
  if (opts & CK_MSG_IMMEDIATE)
  {
...
static inline void _sendMsgNodeBranchMulti(int eIdx, void* msg, CkGroupID gID, int npes,
                                           const int* nodes)
{
...
void CkSendMsgNodeBranchMultiImmediate(int eIdx, void* msg, CkGroupID gID, int npes,
                                       const int* nodes)
{
#if CMK_IMMEDIATE_MSG && !CMK_SMP
...
void CkSendMsgNodeBranchMulti(int eIdx, void* msg, CkGroupID gID, int npes,
                              const int* nodes, int opts)
{
  if (opts & CK_MSG_IMMEDIATE)
  {
...
public:
  ...
  void addLocation(CkLocation& loc) { loc.destroyAll(); }
};
...
void CkDeleteChares()
{
  ...
  {
    IrrGroup* obj =
        CkpvAccess(_groupTable)->find((*CkpvAccess(_groupIDTable))[i]).getObj();
    if (obj && obj->isLocMgr())
    {
...
void registerArrayBcastRecvExtCallback(void (*cb)(int, int, int, int, int*, int, int,
                                                  char*, int))
{
...
void registerCreateCallbackMsgExtCallback(void (*cb)(void*, int, int, int, int*, char**,
                                                     int*))
{
...
MainchareExt::MainchareExt(CkArgMsg* m)
{
  ...
  MainchareCtorExtCallback(thishandle.onPE, thishandle.objPtr, ctorEpIdx, m->argc,
                           m->argv);
  delete m;
}
...
// TODO options
int CkCreateArrayExt(int cIdx, int ndims, int* dims, int eIdx, int num_bufs, char** bufs,
                     int* buf_sizes, int map_gid, char useAtSync, int otherIdx)
{
  // static_cast<void>(impl_e_opts);
  CkAssert(num_bufs >= 1);
  int totalSize = 0;
  ...
  if (otherIdx != -1)
  {
    CkGroupID other_group;
    other_group.idx = otherIdx;
    CkArrayID id(other_group);
    opts.bindTo(id);
  }
  if (map_gid >= 0)
  {
  ...
  CkGroupID gId =
      CProxyElement_ArrayElement::ckCreateArray((CkArrayMessage*)impl_msg, eIdx, opts);
  return gId.idx;
}
...
{
  // printf("[charm] CkMigrateMeExt called with aid: %d, ndims: %d, index: %d, toPe:
...
void CkChareExtSend_multi(int onPE, void* objPtr, int epIdx, int num_bufs, char** bufs,
                          int* buf_sizes)
{
  CkAssert(num_bufs >= 1);
  int totalSize = 0;
...
void CkGroupExtSend_multi(int gid, int npes, const int* pes, int epIdx, int num_bufs,
                          char** bufs, int* buf_sizes)
{
  CkAssert(num_bufs >= 1);
  int totalSize = 0;
  for (int i = 0; i < num_bufs; i++) totalSize += buf_sizes[i];
...
void CkArrayExtSend_multi(int aid, int* idx, int ndims, int epIdx, int num_bufs,
                          char** bufs, int* buf_sizes)
{
  CkAssert(num_bufs >= 1);
  int totalSize = 0;
...
void CkTraceBeginUserBracketEvent(int eventID) { traceBeginUserBracketEvent(eventID); }

void CkTraceEndUserBracketEvent(int eventID) { traceEndUserBracketEvent(eventID); }
...
#if CMK_REPLAYSYSTEM
static FILE* openReplayFile(const char* prefix, const char* suffix,
...
public:
  CkMessageRecorder(FILE* f_) : curpos(0), firstOpen(true), buffer(_recplay_logsize)
  {
    f = f_;
  }
  ~CkMessageRecorder()
...
private:
  ...
  virtual bool process(envelope** envptr, CkCoreState* ck)
  {
    if ((*envptr)->getEvent())
    {
      ...
      if (!wasPacked)
        CkPackMessage(envptr);
      envelope* env = *envptr;
      unsigned int crc1 = 0, crc2 = 0;
      if (_recplay_crc)
      {
        // unsigned int crc = crc32_initial(((unsigned char*)env)+CmiMsgHeaderSizeBytes,
        // env->getTotalsize()-CmiMsgHeaderSizeBytes);
        crc1 = crc32_initial(((unsigned char*)env) + CmiMsgHeaderSizeBytes,
                             sizeof(*env) - CmiMsgHeaderSizeBytes);
        crc2 = crc32_initial(((unsigned char*)env) + sizeof(*env),
                             env->getTotalsize() - sizeof(*env));
      }
      else if (_recplay_checksum)
      {
        crc1 = checksum_initial(((unsigned char*)env) + CmiMsgHeaderSizeBytes,
                                sizeof(*env) - CmiMsgHeaderSizeBytes);
        crc2 = checksum_initial(((unsigned char*)env) + sizeof(*env),
                                env->getTotalsize() - sizeof(*env));
      }
      curpos += snprintf(
          &buffer[curpos], buffer.size() - curpos, "%d %d %d %d %x %x %d\n",
          env->getSrcPe(), env->getTotalsize(), env->getEvent(),
          env->getMsgtype() == NodeBocInitMsg || env->getMsgtype() == ForNodeBocMsg, crc1,
          crc2, env->getEpIdx());
      if (curpos > _recplay_logsize - 128)
        flushLog();
      if (!wasPacked)
        CkUnpackMessage(envptr);
    }
    return true;
  }
  ...
  virtual bool process(LBMigrateMsg** msg, CkCoreState* ck)
  {
    FILE* f;
    if (firstOpen)
      f = openReplayFile("ckreplay_", ".lb", "w");
    else
      f = openReplayFile("ckreplay_", ".lb", "a");
    firstOpen = false;
    if (f != NULL)
    {
      PUP::toDisk p(f);
      p | (*msg)
              ->n_moves;  // Need to store to be able to reload the message during replay
      (*msg)->pup(p);
      fclose(f);
    }
    return true;
...
class CkMessageReplay : public CkMessageWatcher
{
  ...
  void getNext(void)
  {
    ...
    /*
        if (6!=fscanf(f,"%d%d%d%d%x%x",
       &nextPE,&nextSize,&nextEvent,&nexttype,&crc1,&crc2)) { CkAbort("CkMessageReplay>
       Syntax error reading replay file"); nextPE=nextSize=nextEvent=nexttype=-1; //No
       destructor->record file just ends in the middle!
        }
        */
    counter++;
  }
  ...
  {
    if (nextPE != env->getSrcPe())
      return false;
    if (nextEvent != env->getEvent())
      return false;
    if (nextSize < 0)
      return false;  // not waiting for a regular message
#  if 1
    if (nextEP != env->getEpIdx())
    {
      CkPrintf(
          "[%d] CkMessageReplay> Message EP changed during replay org: [%d %d %d %d] "
          "got: [%d %d %d %d]\n",
          CkMyPe(), nextPE, nextSize, nextEvent, nextEP, env->getSrcPe(),
          env->getTotalsize(), env->getEvent(), env->getEpIdx());
      return false;
    }
#  endif
    ...
    {
      CkPrintf(
          "[%d] CkMessageReplay> Message size changed during replay org: [%d %d %d %d] "
          "got: [%d %d %d %d]\n",
          CkMyPe(), nextPE, nextSize, nextEvent, nextEP, env->getSrcPe(),
          env->getTotalsize(), env->getEvent(), env->getEpIdx());
      return false;
    ...
    if (_recplay_crc || _recplay_checksum)
    {
      ...
      if (_recplay_crc)
      {
        // unsigned int crcnew = crc32_initial(((unsigned
        // char*)env)+CmiMsgHeaderSizeBytes, env->getTotalsize()-CmiMsgHeaderSizeBytes);
        unsigned int crcnew1 =
            crc32_initial(((unsigned char*)env) + CmiMsgHeaderSizeBytes,
                          sizeof(*env) - CmiMsgHeaderSizeBytes);
        unsigned int crcnew2 = crc32_initial(((unsigned char*)env) + sizeof(*env),
                                             env->getTotalsize() - sizeof(*env));
        if (crcnew1 != crc1)
        {
          CkPrintf(
              "CkMessageReplay %d> Envelope CRC changed during replay org: [0x%x] got: "
              "[0x%x]\n",
              CkMyPe(), crc1, crcnew1);
        }
        if (crcnew2 != crc2)
        {
          CkPrintf(
              "CkMessageReplay %d> Message CRC changed during replay org: [0x%x] got: "
              "[0x%x]\n",
              CkMyPe(), crc2, crcnew2);
        }
      }
      ...
      {
        unsigned int crcnew1 =
            checksum_initial(((unsigned char*)env) + CmiMsgHeaderSizeBytes,
                             sizeof(*env) - CmiMsgHeaderSizeBytes);
        unsigned int crcnew2 = checksum_initial(((unsigned char*)env) + sizeof(*env),
                                                env->getTotalsize() - sizeof(*env));
        if (crcnew1 != crc1)
        {
          CkPrintf(
              "CkMessageReplay %d> Envelope Checksum changed during replay org: [0x%x] "
              "got: [0x%x]\n",
              CkMyPe(), crc1, crcnew1);
        }
        if (crcnew2 != crc2)
        {
          CkPrintf(
              "CkMessageReplay %d> Message Checksum changed during replay org: [0x%x] "
              "got: [0x%x]\n",
              CkMyPe(), crc2, crcnew2);
        }
      }
  ...
  {
    if (nextPE == CkMyPe() && nextSize == -2 && nextEvent == token->serialNo)
      return true;
    return false;
  }
  ...
  /// This is a (short) list of tokens (i.e messages that awake user-threads) we aren't
  /// yet ready for:
    ...
    if (nextSize > 0)
    {
      ...
      for (int i = 0; i < len; i++)
      {
        ...
        { /* this is the next message: process it */
          REPLAYDEBUG("Dequeueing message: " << env->getSrcPe() << " "
                                             << env->getTotalsize() << " "
                                             << env->getEvent())
          CsdEnqueueLifo(
              (void*)env);  // Make it at the beginning since this is the one we want next
          return;
        }
        else /* Not ready yet-- put it back in the
                queue */
        {
          REPLAYDEBUG("requeueing delayed message: "
                      << env->getSrcPe() << " " << env->getTotalsize() << " "
                      << env->getEvent() << " ep:" << env->getEpIdx())
...
public:
  CkMessageReplay(FILE* f_) : lbFile(NULL)
  {
    ...
    f = f_;
    getNext();
    REPLAYDEBUG("Constructing ckMessageReplay: " << nextPE << " " << nextSize << " "
                                                 << nextEvent);
#  if CMI_QD
    if (CkMyPe() == 0)
      CmiStartQD((CcdCondFn)CkMessageReplayQuiescence, this);
#  endif
  }
...
private:
  virtual bool process(envelope** envptr, CkCoreState* ck)
  {
    ...
    REPLAYDEBUG("ProcessMessage message: "
                << env->getSrcPe() << " " << env->getTotalsize() << " " << env->getEvent()
                << " " << env->getMsgtype() << " " << env->getMsgIdx()
                << " ep:" << env->getEpIdx());
    if (env->getEvent() == 0)
      return true;
    if (isNext(env))
    { /* This is the message we were expecting */
      REPLAYDEBUG("Executing message: " << env->getSrcPe() << " " << env->getTotalsize()
                                        << " " << env->getEvent())
...
#  endif
    ...
    {
      REPLAYDEBUG("Queueing message: " << env->getSrcPe() << " " << env->getTotalsize()
                                       << " " << env->getEvent() << " " << env->getEpIdx()
                                       << " because we wanted " << nextPE << " "
                                       << nextSize << " " << nextEvent << " " << nextEP)
      delayedMessages.enq(env);
      flush();
      return false;
    }
  ...
  virtual bool process(CthThreadToken* token, CkCoreState* ck)
  {
    ...
    {
      REPLAYDEBUG("Queueing token: " << token->serialNo << " because we wanted " << nextPE
                                     << " " << nextSize << " " << nextEvent)
      delayedTokens.enq(token);
      return false;
    }
  ...
  {
    if (lbFile == NULL)
      lbFile = openReplayFile("ckreplay_", ".lb", "r");
    if (lbFile != NULL)
    {
      int num_moves = 0;
...
public:
  ...
  CkMessageDetailReplay(FILE* f_)
  {
    ...
    CcdCallOnCondition(CcdPROCESSOR_STILL_IDLE, (CcdCondFn)CkMessageDetailReplayDone,
                       (void*)this);
  }
...
#endif
  /* For Record/Replay debugging: need to notify the upper layer that we are resuming a
   * thread */
...
void CkMessageWatcherInit(char** argv, CkCoreState* ck)
{
  ...
  char* procs = NULL;
  _replaySystem = 0;
  if (CmiGetArgFlagDesc(argv, "+recplay-crc",
                        "Enable CRC32 checksum for message record-replay"))
  {
    if (CmiMyRank() == 0)
      _recplay_crc = 1;
  }
  if (CmiGetArgFlagDesc(argv, "+recplay-xor",
                        "Enable simple XOR checksum for message record-replay"))
  {
    if (CmiMyRank() == 0)
      _recplay_checksum = 1;
  }
  int tmplogsize;
  if (CmiGetArgIntDesc(argv, "+recplay-logsize", &tmplogsize,
                       "Specify the size of the buffer used by the message recorder"))
  {
    if (CmiMyRank() == 0)
      _recplay_logsize = tmplogsize;
  }
  REPLAYDEBUG("CkMessageWatcherInit ");
  if (CmiGetArgStringDesc(argv, "+record-detail", &procs,
                          "Record full message content for the specified processors"))
  {
#if CMK_REPLAYSYSTEM
    ...
    if (list.includes(CkMyPe()))
    {
      ...
      ck->addWatcher(
          new CkMessageDetailRecorder(openReplayFile("ckreplay_", ".detail", "w")));
    }
#else
    CkAbort(
        "Option `+record-detail' requires that record-replay support be enabled at "
        "configure time (--enable-replay)");
#endif
  }
  if (CmiGetArgFlagDesc(argv, "+record", "Record message processing order"))
  {
    ...
    if (CkMyPe() == 0)
    {
      ...
      {
        CmiPrintf(
            "Charm++> Warning: disabling recording for message integrity detection "
            "(requires linking with -memory charmdebug)\n");
        _recplay_crc = _recplay_checksum = 0;
      }
    }
    CpdSetInitializeMemory(1);
    CmiNumberHandler(CpvAccess(CthResumeNormalThreadIdx),
                     (CmiHandler)CthResumeNormalThreadDebug);
    ck->addWatcher(new CkMessageRecorder(openReplayFile("ckreplay_", ".log", "w")));
#else
    CkAbort(
        "Option `+record' requires that record-replay support be enabled at configure "
        "time (--enable-replay)");
#endif
  }
  if (CmiGetArgStringDesc(
          argv, "+replay-detail", &procs,
          "Replay the specified processors from recorded message content"))
  {
#if CMK_REPLAYSYSTEM
    forceReplay = true;
    CpdSetInitializeMemory(1);
...
#  endif
    _replaySystem = 1;
    ck->addWatcher(
        new CkMessageDetailReplay(openReplayFile("ckreplay_", ".detail", "r")));
#else
    CkAbort(
        "Option `+replay-detail' requires that record-replay support be enabled at "
        "configure time (--enable-replay)");
#endif
  }
  if (CmiGetArgFlagDesc(argv, "+replay", "Replay recorded message stream") || forceReplay)
  {
    ...
    if (CkMyPe() == 0)
    {
      ...
      {
        CmiPrintf(
            "Charm++> Warning: disabling message integrity detection during replay "
            "(requires linking with -memory charmdebug)\n");
        _recplay_crc = _recplay_checksum = 0;
      }
    }
...
#else
    CkAbort(
        "Option `+replay' requires that record-replay support be enabled at configure "
        "time (--enable-replay)");
#endif
  }
  if (_recplay_crc && _recplay_checksum)
  {
    CmiAbort(
        "Both +recplay-crc and +recplay-checksum options specified, only one allowed.");
  }
}
...
int getCharmEnvelopeSize() { return sizeof(envelope); }

/// Best-effort guess at whether @arg msg points at a charm envelope
int isCharmEnvelope(void* msg)
{
  envelope* e = (envelope*)msg;
  if (SIZEFIELD(msg) < sizeof(envelope))
    return 0;
  if (SIZEFIELD(msg) < e->getTotalsize())
    return 0;
  if (e->getTotalsize() < sizeof(envelope))
    return 0;
  if (e->getEpIdx() <= 0 || e->getEpIdx() >= _entryTable.size())
    return 0;
#if CMK_SMP
  if (e->getSrcPe() >= CkNumPes() + CkNumNodes())
    return 0;
#else
  if (e->getSrcPe() >= CkNumPes())
    return 0;
#endif
  if (e->getMsgtype() <= 0 || e->getMsgtype() >= LAST_CK_ENVELOPE_TYPE)
    return 0;
  return 1;
}
