...
static void bdcastROGroupData(void)
{
  ...
  char* msg =
      (char*)CmiAlloc(CmiMsgHeaderSizeBytes + 2 * sizeof(int) + ps.size() + ps1.size());
                                ...
                                const char* mode, const int id = -1)
{
  ...
  if (!fp)
  {
    CkAbort("PE %d failed to open checkpoint file: %s, mode: %s, status: %s", CkMyPe(),
            filename.c_str(), mode, strerror(errno));
  }
  return fp;
}
...
public:
                  ...
                  int writersPerNode = 0)
  {
    ...
    if (writersPerNode > 0)
      numWriters = std::min(writersPerNode, nodeSize);
...
void CkPupROData(PUP::er& p)
{
  ...
  if (!p.isUnpacking())
    _numReadonlies = _readonlyTable.size();
  ...
  if (!p.isUnpacking())
    _numReadonlyMsgs = _readonlyMsgs.size();
  p | _numReadonlyMsgs;
  ...
  for (i = 0; i < n; i++)
  {
    ...
    if (pup_flag)
    {
      if (p.isUnpacking())
      {
        ...
        {
          CkAbort(
              "Chare %s needs a migration constructor and PUP'er routine for restart.\n",
                              ...
                              GroupCreationFn creationFn)
{
  ...
  if (!p.isUnpacking())
  {
    for (i = 0; i < numGroups; i++)
    {
      ...
      {
        CkAbort(
            "(Node)Group %s needs a migration constructor and PUP'er routine for "
            "restart.\n",
...
{
  CkPupPerPlaceData(p, &CksvAccess(_nodeGroupIDTable), CksvAccess(_nodeGroupTable),
                    CksvAccess(_numNodeGroups), NodeBocInitMsg, &CkCreateLocalNodeGroup);
}
...
void CkPupArrayElementsData(PUP::er& p, int notifyListeners)
{
  ...
  DEBCHK("[%d] CkPupArrayElementsData %s numGroups:%d numElements:%d \n", CkMyPe(),
         p.typeString(), numGroups, numElements);
...
void CkTestArrayElements()
{
  ...
  for(i=0; i<numGroups; i++){
    IrrGroup *obj =
CkpvAccess(_groupTable)->find((*CkpvAccess(_groupIDTable))[i]).getObj(); CkPrintf("An
object at [%d]: %p | isLocMgr: %d\n", i, obj, obj->isLocMgr());
  }
}
*/
...
void CkRestartMain(const char* dirname, CkArgMsg* args)
{
  ...
  // restore groups
  // content of the file: numGroups, GroupInfo[numGroups], _groupTable(PUP'ed),
  // groups(PUP'ed) restore from PE0's copy if shrink/expand
  FILE* fGroups =
      openCheckpointFile(dirname, "Groups", "rb", (CkNumPes() == _numPes) ? CkMyPe() : 0);
  ...
  if (CmiMyRank() == 0)
    _initDone();  // this rank will trigger other ranks
                  //_initDone();
