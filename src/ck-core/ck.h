...
inline bool _tryIpcSend(int dst, envelope* env, int infofn)
{
  ...
  else if ((dst == CLD_ANYWHERE) || (dst == CLD_BROADCAST) || (dst == CLD_BROADCAST_ALL))
  {
    return false;
  }
...
public:
   ...
   * up to the outermost
   */
#define PROCESS_MACRO(name, type)                         \
  inline bool process##name(type* input, CkCoreState* ck) \
  {                                                       \
    bool result = true;                                   \
    if (next != NULL)                                     \
      result &= next->process##name(input, ck);           \
...
public:
  ...
  CkCoreState() : groupTable(CkpvAccess(_groupTable)), qd(CpvAccess(_qd))
  {
    watcher = NULL;
  }
  ~CkCoreState() { delete watcher; }

  inline GroupTable* getGroupTable() const { return groupTable; }
...
extern void _infoFn(void* msg, CldPackFn* pfn, int* len, int* queueing, int* priobits,
                    UInt** prioptr);
