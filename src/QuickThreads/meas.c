...
typedef struct foo_t {
    int iter;           /* Number of times to perform test. */
...
static foo_t foo[] = {
  ...
  { 10000000, test01_msg, test01_descr, test01 },
  {  1000000, test02_msg, NULL, test02 },
  {  1000000, test03_msg, NULL, test03 },
  {  1000000, test04_msg, NULL, test04 },
  {  1000000, test05_msg, NULL, test05 },
  {  1000000, test06_msg, test06_descr, test06 },
  {  1000000, test07_msg, test07_descr, test07 },
  {  1000000, test08_msg, test08_descr, test08 },
  {  1000000, test09_msg, NULL, test09 },
  {  1000000, test10_msg, test10_descr, test10 },
  {  1000000, test11_msg, test11_descr, test11 },
  {  1000000, test12_msg, test12_descr, test12 },
  {  1000000, test13_msg, test13_descr, test13 },
  {  1000000, test14_msg, test14_descr, test14 },
  {  1000000, test15_msg, test15_descr, test15 },
  {  1000000, test16_msg, test16_descr, test16 },
  {  1000000, test17_msg, test17_descr, test17 },
  { 10000000, test18_msg, test18_descr, test18 },
  { 10000000, test19_msg, test19_descr, test19 },
  { 50000000, test20_msg, test20_descr, test20 },
  { 50000000, test21_msg, test21_descr, test21 },
  { 0, 0 }
};
...
usage()
{
  ...
  void
args (int *which, int argc, char **argv)
{
  static int nfuncs = 0;
...
{
  int which; double init, final;
  struct timeval tv;
  args (&which, argc, argv);
  gettimeofday(&tv, 0);
  init = tv.tv_sec * 1.0E6 + tv.tv_usec;
  (*(foo[which].f))(foo[which].iter);
  gettimeofday(&tv, 0);
  final = tv.tv_sec * 1.0E6 + tv.tv_usec;
  fprintf(stderr,"Test %d %.4f microsec\n", which, ((final-init)/foo[which].iter));
  exit (0);
  return (0);
}
