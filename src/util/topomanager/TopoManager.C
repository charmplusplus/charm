...
struct CompareRankDist
{
  ...
  bool operator()(int i, int j) { return (peDist[i] < peDist[j]); }
};
...
TopoManager::TopoManager(int NX, int NY, int NZ, int NT)
    : dimNX(NX), dimNY(NY), dimNZ(NZ), dimNT(NT)
{
...
{
  if (!(x >= 0 && x < dimNX && y >= 0 && y < dimNY && z >= 0 && z < dimNZ && t >= 0 &&
        t < dimNT))
    return -1;
  ...
  {
    if (t >= CmiNumPesOnPhysicalNode(x))
      return -1;
    else
      return CmiGetFirstPeOnPhysicalNode(x) + t;
  }
#endif
}
...
{
  int root_coords[4];
  rankToCoordinates(pe, root_coords[0], root_coords[1], root_coords[2], root_coords[3]);
...
int TopoManager::areNeighbors(int pe1, int pe2, int pe3, int distance) const
{
  ...
  if ((absX(pe1_x - (pe2_x + pe3_x) / 2) + absY(pe1_y - (pe2_y + pe3_y) / 2) +
       absZ(pe1_z - (pe2_z + pe3_z) / 2)) <= distance)
    return 1;
  else
    return 0;
}
...
void TopoManager::printAllocation(FILE* fp) const
{
  ...
  for (i = 0; i < numPes; i++)
  {
    ...
    fprintf(fp, "%d/%d - %d/%d - %d %d %d %d\n", CmiGetPeGlobal(i, CmiMyPartition()),
            CmiGetNodeGlobal(CmiNodeOf(i), CmiMyPartition()), i, CmiNodeOf(i), x, y, z,
            t);
  }
}
...
#ifdef __TPM_STANDALONE__
extern "C" void TopoManager_setNumThreads(int t) { _tpm_numthreads = t; }
#endif
...
extern "C" void TopoManager_printAllocation(FILE* fp)
{
  ...
  if (_tmgr == NULL)
  {
    printf("ERROR: TopoManager NOT initialized. Aborting...\n");
    exit(1);
  }
#endif
...
extern "C" void TopoManager_getDimCount(int* ndims) { *ndims = 3; }
...
extern "C" void TopoManager_createPartitions(int scheme, int numparts, int* nodeMap)
{
  if (scheme == 0)
  {
    ...
    {
      printf(
          "Charm++> Using rank ordered division (scheme 0) for topology aware "
          "partitions\n");
    }
    int i;
  ...
  // else if(scheme == 1) {
  //   if(!CmiMyNodeGlobal()) {
  //     printf("Charm++> Using planar division (scheme 1) for topology aware
  //     partitions\n");
  //   }
  //   getPlanarList(nodeMap);
  // } else if(scheme == 2) {
  //   if(!CmiMyNodeGlobal()) {
  //     printf("Charm++> Using hilber curve (scheme 2) for topology aware
  //     partitions\n");
  //   }
  //   getHilbertList(nodeMap);
  // } else if(scheme == 3) {
  //   if(!CmiMyNodeGlobal()) {
  //     printf("Charm++> Using recursive bisection (scheme 3) for topology aware
  //     partitions\n");
  //   }
