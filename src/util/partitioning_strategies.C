...
struct TopoManagerWrapper
{
  ...
  TopoManagerWrapper()
  {
    ...
    for (int node = 0; node < nnodes; ++node)
    {
      ...
      e = 0;
      if (a < 0 || a >= na)
        CmiAbort("inconsistent torus topology!");
      if (b < 0 || b >= nb)
        CmiAbort("inconsistent torus topology!");
      if (c < 0 || c >= nc)
        CmiAbort("inconsistent torus topology!");
...
#  endif
    ...
    // lensort[1] & 7, lensort[2] & 7, lensort[3] & 7, lensort[4] & 7);
    ...
    iout << "Charm++> " << "TORUS MINIMAL MESH SIZE IS " << a_len << " BY " << b_len
         << " BY " << c_len << "\n";
    if (CmiMyNodeGlobal() == 0)
      printf("%s", iout.str().c_str());
    // printf("TopoManagerWrapper dims %d %d %d %d %d\n", a_dim, b_dim, c_dim, d_dim,
    // e_dim);
  }
  ...
  void sortLongest(int* node_begin, int* node_end)
  {
    ...
    if (PARTITION_TOPOLOGY_VERBOSE && CmiMyNodeGlobal() == 0)
      printf("sorting %d(%d) %d(%d) %d(%d)\n", sortdims[0], tlens[4] >> 3, sortdims[1],
             tlens[3] >> 3, sortdims[2], tlens[2] >> 3);
    std::sort(node_begin, node_end, node_sortop_topo(*this, sortdims));
  }
};

void recursive_bisect(int part_begin, int part_end, int* node_begin, int* node_end,
                      TopoManagerWrapper& tmgr)
{
  if (part_end - part_begin == 1)
  {
    ...
    if (PARTITION_TOPOLOGY_VERBOSE && CmiMyNodeGlobal() == 0)
    {
      ...
      printf("partitioning node %5d at %5d %5d %5d %5d %5d nodes %5" PRId64 "\n",
             *node_begin, crds[0], crds[1], crds[2], crds[3], crds[4],
  ...
  for (int p = part_begin; p < part_end; ++p)
  {
    ...
    if (abs(ncount + ps - nsplit) < abs(ncount - nsplit))
      part_split = p + 1;
    else
      break;
    ncount += ps;
  ...
  // recurse
  recursive_bisect(part_begin, part_split, node_begin, node_split, tmgr);
  recursive_bisect(part_split, part_end, node_split, node_end, tmgr);
}
...
//     return;
//   TopoManagerWrapper tmgr;
//   recursive_bisect(0, numparts, procList, procList + n, tmgr);
//   if (PARTITION_TOPOLOGY_VERBOSE && CmiMyNodeGlobal() == 0)
//   {
//     int crds[5];
