...
public:
  PhyNode(int id, int pe, TopoManager* tmgr) : id(id), pe(pe)
  {
    if (tmgr->haveTopologyInfo())
      tmgr->rankToCoordinates(pe, coords);
  }
    ...
    : nodeTree(nodeTree), preSorted(preSorted)
{
  ...
  if (tmgr->haveTopologyInfo())
  {
    for (int i = 0; i < tmgr->getNumDims(); i++)
    {
      if (tmgr->getDimSize(i) > DIM_SET_SIZE)
        CkAbort(
            "ST_RecursivePartition:: Increase bitset size to match size of largest "
            "topology dimension");
    }
  }
#if _DEBUG_SPANNING_TREE_
  if (CkMyNode() == 0)
  {
    CkPrintf("TopoManager reports topoinfo=%d, %d dims, dim sizes: ",
             tmgr->haveTopologyInfo(), tmgr->getNumDims());
                                                       ...
                                                       unsigned int maxBranches)
{
  ...
  if (numNodes == 0)
    CkAbort("Error: requested spanning tree but no nodes\n");
  else if (numNodes == 1)
    return 0;

#if _DEBUG_SPANNING_TREE_
  CkPrintf(
      "[%d] ST_RecursivePartition:: Root is %d, being requested %d children, Num nodes "
      "incl root is %d\n",
  ...
  std::vector<typename ST_RecursivePartition<Iterator>::PhyNode*> pphynodes(
      phynodes.size());
                                                   ...
                                                   std::vector<PhyNode>& phynodes) const
{
  ...
  int rootPhyNodeId;
  if (nodeTree)
    rootPhyNodeId = CmiPhysicalNodeID(CmiNodeFirst(*start));
  else
    rootPhyNodeId = CmiPhysicalNodeID(*start);  // contains pes
...
#if _DEBUG_SPANNING_TREE_
    if (phyNodeId == rootPhyNodeId)
      CkPrintf("[%d] Node %d is in root phynode\n", CkMyNode(), n);
#endif
...
template <typename Iterator>
void ST_RecursivePartition<Iterator>::withinPhyNodeTree(PhyNode& rootPhyNode, int bfactor,
                                                        Iterator& pos)
{
  if (rootPhyNode.size() == 1)
    return;  // only one element in physical node

  std::vector<int> nodes;  // nodes in this phynode (root is ignored)
  std::map<int, std::vector<int>>
      nodePes;  // PEs in each node (used when building PE tree)
  if (nodeTree)
    nodes.assign(rootPhyNode.nodes.begin() + 1, rootPhyNode.nodes.end());
  else
  {
  ...
  else
  {
    ...
    for (std::vector<int>::iterator i = nodes.begin(); parts < numChildren;
         i += partSize, parts++)
    {
      children.push_back(pos);
      std::vector<int>::iterator end;
      if (parts == numChildren - 1)
        end = nodes.end();
      else
        end = i + partSize;
...
void ST_RecursivePartition<Iterator>::build(std::vector<PhyNode*>& phyNodes,
                                            Iterator start, unsigned int maxBranches)
{
  typename ST_RecursivePartition<Iterator>::PhyNode* rootPhyNode = phyNodes[0];
  children.reserve(rootPhyNode->size() +
                   maxBranches);  // reserve for max number of children
  ...
  if (tmgr->haveTopologyInfo())
    // choose root phynode in each subtree (closest one to top-level root phynode), put at
    // beginning
  ...
  {
    children.push_back(pos);
    for (int j = phyNodeChildren[i]; j < phyNodeChildren[i + 1]; j++)
    {  // for each phynode in subtree
      for (int k = 0; k < phyNodes[j]->size(); k++)
      {  // for each node in phynode
                                                         ...
                                                         std::vector<int>& children) const
{
  ...
  {                           // for each subtree
    ...
    int closestIdx = -1;
    for (int j = start; j < children[i + 1]; j++)
    {  // for each phynode in subtree
...
#if _DEBUG_SPANNING_TREE_
    if (CkMyNode() == 0)
      CkPrintf("Subtree %d, closest phynode to root is %d, distance=%d\n", i,
               phyNodes[closestIdx]->id, minDistance);
#endif
...
template <typename Iterator>
void ST_RecursivePartition<Iterator>::partition(std::vector<PhyNode*>& nodes, int start,
                                                int end, int numPartitions,
                                                std::vector<int>& children) const
{
#if _DEBUG_SPANNING_TREE_
  ...
  if ((numPartitions > 1) && (numNodes > 1))
  {
    ...
    if (numPartitions % 3 == 0)
      trisect(nodes, start, end, numPartitions, children);
    else
      bisect(nodes, start, end, numPartitions, children);
  }
  ...
  else
  {
    ...
    CkAbort(
        "\nPartitioning fell through to the default case (which it never should). Check "
        "the logic in this routine.");
  }
}

template <typename Iterator>
void ST_RecursivePartition<Iterator>::bisect(std::vector<PhyNode*>& nodes, int start,
                                             int end, int numPartitions,
                                             std::vector<int>& children) const
{
  ...
  if (tmgr->haveTopologyInfo())
  {
    ...
    std::nth_element(itr + start, itr + median, itr + end,
                     typename ST_RecursivePartition::PhyNodeCompare(maxSpreadDim));
#if _DEBUG_SPANNING_TREE_
...
template <typename Iterator>
void ST_RecursivePartition<Iterator>::trisect(std::vector<PhyNode*>& nodes, int start,
                                              int end, int numPartitions,
                                              std::vector<int>& children) const
{
  const int numNodes = end - start;
  ...
  if (tmgr->haveTopologyInfo())
  {
    int maxSpreadDim = maxSpreadDimension(nodes, start, end);
    typename std::vector<PhyNode*>::iterator itr = nodes.begin();
    std::nth_element(itr + start, itr + oneThird, itr + end,
                     typename ST_RecursivePartition::PhyNodeCompare(maxSpreadDim));
    std::nth_element(itr + oneThird, itr + twoThird, itr + end,
                     typename ST_RecursivePartition::PhyNodeCompare(maxSpreadDim));
#if _DEBUG_SPANNING_TREE_
                                                        ...
                                                        int start, int end) const
{
  ...
  if (!tmgr->haveTopologyInfo() || (nDims <= 1))
    return 0;
 ...
 * In other words, "moves" all phynodes by some translation offset so that their bounding
 * box includes minimum number of coordinates, which implies that adjacent nodes won't go
 * through torus edges. Works for any number of dimensions (N-d torus)
 */
template <typename Iterator>
void ST_RecursivePartition<Iterator>::translateCoordinates(
    std::vector<PhyNode>& nodes) const
{
  const int nDims = tmgr->getNumDims();
  ...
  for (int i = 0; i < nDims; i++)
  {
    ...
    {
      if (!usedCoordinates[i].test(j))
      {  // j coordinate not used by any element
        sum += j;
        nbUnusedCoords++;
      }
    }
    if (nbUnusedCoords > 0)
      gapCenter[i] = sum / nbUnusedCoords;
  }
...
#  endif
  ...
  for (int i = 0; i < nDims; i++)
  {  // find best translation offset for each dimension
    if (maxSpread[i] == int(usedCoordinates[i].count()))
      continue;  // nothing to correct for this dimension

#  if _DEBUG_SPANNING_TREE_
    if (CkMyNode() == 0)
      CkPrintf("Going to attempt to correct coordinates on dimension %d\n", i);
#  endif

    // choose direction of unused coordinates to finish faster
    int direction = 1;  // go "right"
    if (gapCenter[i] < dimSizes[i] / 2)
      direction = -1;  // go "left"
#  if _DEBUG_SPANNING_TREE_
    if (CkMyNode() == 0)
      CkPrintf("Chose direction %d\n", direction);
#  endif
...
#  if _DEBUG_SPANNING_TREE_
        if (CkMyNode() == 0)
          CkPrintf("FIXED after %d movements\n", m + 1);
#  endif
        break;
      }
      ...
      if (m == dimSizes[i] - 2)
      {
        // did max number of possible movements (another movement would return us to
        // original coordinates/offset), exit loop
...
#  if _DEBUG_SPANNING_TREE_
        if ((CkMyNode() == 0) && (bestMaxSpread < maxSpread[i]))
          CkPrintf("Improved to %d max spread\n", bestMaxSpread);
#  endif
                            ...
                            bool nodeTree, unsigned int bfactor, CmiSpanningTreeInfo& t)
{
  ...
  int numSubtrees = tb.buildSpanningTree(
      start, end, std::min(bfactor, (unsigned int)std::distance(start, end) - 1));
  if (myElem == *start)
  {
    // I am the root of this subtree so we're done (collect my children and return)
    t.parent = prevLvlParent;
    if (numSubtrees > 0)
      t.children = (int*)malloc(sizeof(int) * numSubtrees);
  ...
  for (int i = 0; i < numSubtrees; i++)
  {
    ...
    if (f != subtreeEnd)
    {
      getNeighborsTopoTree_R(subtreeStart, subtreeEnd, myElem, *start, nodeTree, bfactor,
                             t);
      elemFound = true;
      break;
    }
  ...
  if (!elemFound)
    CkAbort("Can't build tree. The element is not in the list of tree nodes");
}
...
void getNodeTopoTreeEdges(int node, int rootNode, int* nodes, int numnodes,
                          ...
                          int** children)
{
  CkAssert((node >= 0 && node < CkNumNodes()) &&
           (rootNode >= 0 && rootNode < CkNumNodes()) && (bfactor > 0));
  ...
  else
  {
    CkAssert(numnodes > 0);
    if (rootNode != nodes[0])
      CkAbort("getNodeTopoTreeEdges: root must be in first position of nodes");
  }
...
{
  CkAssert((pe >= 0 && pe < CkNumPes()) && (rootPE >= 0 && rootPE < CkNumPes()) &&
           (bfactor > 0));
  ...
  else
  {
    CkAssert(numpes > 0);
    if (rootPE != pes[0])
      CkAbort("getPETopoTreeEdges: root must be in first position of pes");
  }
...
CmiSpanningTreeInfo* ST_RecursivePartition_getTreeInfo(int root)
{
  if (trees.size() == 0)
  {
    ...
    if (CkMyRank() != 0)
      CkAbort("First call to getTreeInfo has to be by rank 0");
#endif
  }
  CmiLock(_treeLock);
  ...
  else
  {
    ...
    t->children = NULL;
    trees[root] = t;
    getNodeTopoTreeEdges(CkMyNode(), root, NULL, -1, 4, &t->parent, &t->child_count,
                         &t->children);
    CmiUnlock(_treeLock);
    return t;
  }
...
void partitionPEs(int* pes, int numpes, int numparts, int* part_offsets)
{
  ...
  {
    CkAbort(
        "partitionPEs: number of partitions found is different than requested number.\n"
        "The number of physical nodes of the given PEs might be less than requested "
        "partitions\n");
  }
  for (int i = 0; i < numSubtrees; i++)
  {
