...
#define DAEMON_MAGIC 0x7AF2893C
	unsigned int magic;/*Magic number for daemon (a weak security measure)*/
	/*Length (bytes) of the program arguments to follow*/
	unsigned int argLength;
	/*These parameters are needed by net-machine.c, which gets them
	  via the NETSTART environment variable.*/
	unsigned int Cmi_numpes;/* Total number of processors */
	unsigned int Cmi_mynodesize;/* Number of processors in my address space */
	unsigned int Cmi_mynode;/* Which address space am I */
	unsigned int Cmi_numnodes;/* Total number of address spaces */
	unsigned int Cmi_nodestart;/* First processor in this address space */
	unsigned int Cmi_self_IP;
	unsigned int Cmi_host_IP;
	unsigned int Cmi_host_port;
	unsigned int Cmi_host_pid;
} taskStruct;
