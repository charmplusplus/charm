...
int  SendSocketN(SOCKET, BYTE*, int);
...
static unsigned int   ctrlport, dataport;
static SOCKET         ctrlskt, dataskt;
...
static volatile int proc_done = 0;
...
static volatile int    barrier_wait0 = 0;
static volatile int    barrier_wait1 = 0;
...
/* Big time socket usage -

  Since NT handles sockets differently (specifically, you can't
  just fdopen and then fgets), we send a byte-count and then
  the command string instead of just the command string followed
  by newline.  OSL 12/4/1999-- NT port
