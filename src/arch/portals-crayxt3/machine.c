...
extern ptl_handle_ni_t _ni_handle;
int mynid, mypid;
...
void ConverseInit(int argc, char **argv, CmiStartFn fn, int usched, int initret)
{
  ...
  _Cmi_numnodes = cnos_get_size();

  _Cmi_mynode = cnos_get_rank();

  (void)cnos_get_nidpid_map( &nidpid );

  if ( PtlInit( &max_ifaces ) != PTL_OK ) {
    fprintf(stderr,"%d: PtlInit() failed\n", CmiMyNode());
    return;
  }

  if ( PtlGetId( _ni_handle, &my_id ) != PTL_OK ) {
    fprintf(stderr,"%d: PtlGetId() failed\n", CmiMyNode());
    return;
  }

  if ( (my_id.nid != nidpid[CmiMyNode()].nid) ||
       (my_id.pid != nidpid[CmiMyNode()].pid) ) {
    fprintf(stderr,"%d: Error: invalid nid/pid map\n", CmiMyNode());
    return;
  }
  mynid = my_id.nid;
  mypid = my_id.pid;
  printf("%d: nidpid is (%d, %d)\n", CmiMyNode(), nidpid[CmiMyNode()].nid, nidpid[CmiMyNode()].pid);
