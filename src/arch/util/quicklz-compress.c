...
CpvDeclare(qlz_state_compress, state_compress);
void quicklz_compress(void *in_data, void *out_data, int in_data_size, int *out_data_size)
{
#if     DEBUG_QUICKLZ
    double start_timer=CmiWallTimer();
#endif
    *out_data_size = qlz_compress(in_data, out_data, in_data_size, &(CpvAccess(state_compress)));
#if     DEBUG_QUICKLZ
    CmiPrintf("Compress on [%d] ori %d===>%d rate:%.5f, time :%.6f\n", CmiMyPe(), in_data_size, *out_data_size,  *out_data_size/(float)in_data_size, CmiWallTimer()-start_timer);
#endif
}

CpvDeclare(qlz_state_decompress, state_decompress);
int quicklz_decompress(const void *src, void *dst, int srcLen, int dstLen) {
    int len;
#if     DEBUG_QUICKLZ
    double start_timer=CmiWallTimer();
#endif
    len = qlz_size_decompressed(src);
    len = qlz_decompress(src, dst, &(CpvAccess(state_decompress)));
#if     DEBUG_QUICKLZ
    CmiPrintf("DeCompress on [%d] time :%.6f\n", CmiMyPe(), CmiWallTimer()-start_timer);
#endif
    return len;
}
...
void quicklz_init()
{
    CpvInitialize(qlz_state_compress, state_compress);
    CpvInitialize(qlz_state_decompress, state_decompress);
}
