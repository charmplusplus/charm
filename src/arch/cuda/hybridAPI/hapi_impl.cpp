...
void hapiCuptiInit() {
  ...
  // CUPTI_ACTIVITY_KIND_RUNTIME must stay enabled even though nothing here
  // consumes its records: EXTERNAL_CORRELATION records are only emitted for
  // correlation IDs generated by runtime-API tracking. Without it, every
  // kernel arrives with no correlation and the load balancer sees zero GPU
  // load for every object.
