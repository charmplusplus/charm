...
struct DeviceManager {
...
#if CMK_LBDB_ON
  // Device properties used to compute per-kernel SM usage from CUPTI records.
  // Populated once per device at init; zero until filled in.
  int multi_processor_count;
  int max_threads_per_sm;
  int max_blocks_per_sm;            // 0 if not available (pre-CUDA 11)
  int max_registers_per_sm;
  int max_shared_mem_per_sm;        // bytes
  int warp_size;
  ...
  DeviceManager(int local_index_, int global_index_) :
    ...
    , multi_processor_count(0), max_threads_per_sm(0), max_blocks_per_sm(0),
      max_registers_per_sm(0), max_shared_mem_per_sm(0), warp_size(32),
