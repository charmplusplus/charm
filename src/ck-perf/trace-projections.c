...
CpvStaticDeclare(FILE*, log_file_desc);
...
char **argv;
{
  ...
  CpvInitialize(FILE*,log_file_desc);
...
{ 
  ...
  if((CpvAccess(log_file_desc)=fopen(CpvAccess(log_file_name), "w")) == NULL)
    CmiPrintf("*** ERROR *** Cannot Create %s",CpvAccess(log_file_name));
  fprintf(CpvAccess(log_file_desc), "PROJECTIONS-RECORD\n");
  CpvAccess(logcnt) = 0; 
  if (pe == 0)
  {
    char *state_file;

    state_file = (char *) CmiAlloc(strlen(CpvAccess(pgm))+strlen(".sts")+1);
...
void close_log(void)
{
  ...
  fclose(CpvAccess(log_file_desc));
  if (pe == 0)
  {
...
int count;
{
  ...
  {
    write_out_projections_line(CpvAccess(log_file_desc), 
          buf->type, buf->msg_type,
          buf->entry, buf->time1,
          buf->event, buf->pe);
    fprintf(CpvAccess(log_file_desc), "\n");
  }
  CpvAccess(logcnt) = 0;
  fflush(CpvAccess(log_file_desc));
}
