...
void DDT_DataType::pup(PUP::er  &p)
{
	p(datatype);
	p(refCount);
	p(size);
...
void DDT_Contiguous::pup(PUP::er &p)
{
	p(size);
	p(extent);
	p(count);
...
void DDT_HIndexed::pup(PUP::er &p)
{
	DDT_Indexed::pup(p);
}
