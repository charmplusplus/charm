...
Isaac Dooley, idooley2@uiuc.edu, 1/25/2005
*/
...
class NetFEM_elems : public NetFEM_item {
  ...
  int cell_type; // VTK cell type as defined in netfem.h

  void allocate(void) {
    isHeapAllocated=true;
    conn=new int[getItems()*nodesPer];
    CkPrintf("Allocating an element conn data: items=%d nodesPer=%d\n",getItems(),nodesPer);
  }
  void localCopy(void);
 ...
 public:
  ...
  NetFEM_elems(int nEl,int nodesPerEl,int bytesPerEl,
	       int idxBase_,int *conn_,const char *name_, int cell_type_)
	...
	// The number of vertices for a cell
	int getNodesPer(){
	  return nodesPer;
	}

    int getCellType() const {return cell_type;}
...
class NetFEM_update {
  ...
  NetFEM_item *cur;
 public:
  NetFEM_update(int src_,int ts_,int dim_)
    :source(src_),timestep(ts_),dim(dim_)
    {
      nodes=NULL;
    ...
    int setPartitions(int nPartitions){nsources=nPartitions;}
    int getSource(void) const {return source;}      // this chunk number
    int getNumChunks(void) const {return nsources;} // total number of chunks/partitions
