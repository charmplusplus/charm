...
void MPIR_MPIOInit(int * error_code) {
    ...
    if (ADIO_Global(ADIO_Init_keyval) == MPI_KEYVAL_INVALID) {
	...
	/* --END ERROR HANDLING-- */
        ...
        MPI_Keyval_create(MPI_NULL_COPY_FN, ADIOI_End_call, &ADIO_Global(ADIO_Init_keyval),
                          (void *) 0);  
	...
	/* put a dummy attribute on MPI_COMM_SELF, because we want the delete
	   ...
	   MPI_COMM_SELF get cleaned up early in MPI_Finalize */
        ...
        MPI_Attr_put(MPI_COMM_SELF, ADIO_Global(ADIO_Init_keyval), (void *) 0);
