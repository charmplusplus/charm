...
{
    if (file_ptr == MPI_FILE_NULL) ADIO_Global(ADIOI_DFLT_ERR_HANDLER) = e;
...
int MPIR_ROMIO_Get_file_errhand( MPI_File file_ptr, MPI_Errhandler *e )
{
	...
	if (ADIO_Global(ADIOI_DFLT_ERR_HANDLER) == MPI_ERRORS_RETURN)
	    *e = 0;
	else {
	    *e = ADIO_Global(ADIOI_DFLT_ERR_HANDLER);
	}
    }
    /* --BEGIN ERROR HANDLING-- */
    else if (file_ptr->cookie != ADIOI_FILE_COOKIE) {
