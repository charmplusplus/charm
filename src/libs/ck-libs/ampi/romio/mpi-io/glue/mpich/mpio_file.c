...
MPI_File MPIO_File_f2c(MPI_Fint fh)
{
    ...
    if ((fh < 0) || (fh > ADIO_Global(ADIOI_Ftable_ptr))) {
	/* there is no way to return an error from MPI_File_f2c */
	return MPI_FILE_NULL;
    }
    return ADIO_Global(ADIOI_Ftable)[fh];
#endif
}
...
MPI_Fint MPIO_File_c2f(MPI_File fh)
{
    ...
    if (!ADIO_Global(ADIOI_Ftable)) {
	ADIO_Global(ADIOI_Ftable_max) = 1024;
	ADIO_Global(ADIOI_Ftable) = (MPI_File *)
	    ADIOI_Malloc(ADIO_Global(ADIOI_Ftable_max)*sizeof(MPI_File)); 
        ADIO_Global(ADIOI_Ftable_ptr) = 0;  /* 0 can't be used though, because 
                                  MPI_FILE_NULL=0 */
	for (i=0; i<ADIO_Global(ADIOI_Ftable_max); i++) ADIO_Global(ADIOI_Ftable)[i] = MPI_FILE_NULL;
    }
    if (ADIO_Global(ADIOI_Ftable_ptr) == ADIO_Global(ADIOI_Ftable_max)-1) {
	ADIO_Global(ADIOI_Ftable) = (MPI_File *) ADIOI_Realloc(ADIO_Global(ADIOI_Ftable), 
                           (ADIO_Global(ADIOI_Ftable_max)+1024)*sizeof(MPI_File));
	for (i=ADIO_Global(ADIOI_Ftable_max); i<ADIO_Global(ADIOI_Ftable_max)+1024; i++) 
	    ADIO_Global(ADIOI_Ftable)[i] = MPI_FILE_NULL;
	ADIO_Global(ADIOI_Ftable_max) += 1024;
    }
    ADIO_Global(ADIOI_Ftable_ptr)++;
    ADIO_Global(ADIOI_Ftable)[ADIO_Global(ADIOI_Ftable_ptr)] = fh;
    fh->fortran_handle = ADIO_Global(ADIOI_Ftable_ptr);
    return (MPI_Fint) ADIO_Global(ADIOI_Ftable_ptr);
#endif
}
