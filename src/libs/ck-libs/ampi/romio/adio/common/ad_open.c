		   ...
		   MPI_Info info, int perm, int *error_code)
{
	...
	*error_code = MPIO_Err_create_code(*error_code,
    ...
    fd->err_handler = ADIO_Global(ADIOI_DFLT_ERR_HANDLER);
...
/* create and initialize info object */
	...
	*error_code = MPIO_Err_create_code(*error_code,
    ...
    if (!can_skip) {
	if (ADIO_Global(ADIOI_syshints) == MPI_INFO_NULL)
	    MPI_Info_create(&ADIO_Global(ADIOI_syshints));
	ADIOI_process_system_hints(fd, ADIO_Global(ADIOI_syshints));
    }

    ADIOI_incorporate_system_hints(info, ADIO_Global(ADIOI_syshints), &dupinfo);
