...
ADIOI_Flatlist_node * ADIOI_Add_contig_flattened(MPI_Datatype contig_type)
{
    ...
    ADIOI_Flatlist_node *flat_node_p = ADIO_Global(ADIOI_Flatlist);
			   ...
			   view_state *client_file_view_state_arr)
{
	...
	filetype_sz = filetype_extent;
        ...
        flat_file_p = ADIO_Global(ADIOI_Flatlist);
    ...
    if (fd->is_agg == 1)
    {
	ADIOI_Flatlist_node *fr_node_p = ADIO_Global(ADIOI_Flatlist);
	for (i = 0; i < nprocs; i++)
	{
