...
namespace xi {
  ...
  void CEntry::generateCode(XStr& decls, XStr& defs) {
    ...
    defs << "    MergeablePathHistory *currentSaved = c->getPath(); /* critical path SYH */ \n";
    defs << "    mergePathHistory(currentSaved); /* critical path SYH */ \n";
#endif
...
#if USE_CRITICAL_PATH_HEADER_ARRAY
    defs << "else {  /* SYH */ \n";
    defs << "    MergeablePathHistory *currentSaved = saveCurrentPath(); /*  SYH */ \n";
    defs << "    buff0->setPath(currentSaved); /*  SYH */ \n";
    defs << "}\n";
#endif
    defs << "}\n\n";
    templateGuardEnd(defs);
