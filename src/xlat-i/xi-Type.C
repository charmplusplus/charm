...
void FuncType::print(XStr& str) {
  rtype->print(str);
  str << "(*" << name << ")(";
  if (params) params->print(str);
  str << ")";
}

void Type::genProxyName(XStr& str, forWhom forElement) {
  (void)str;
  (void)forElement;
  die("type::genProxyName called (INTERNAL ERROR)");
}
void Type::genIndexName(XStr& str) {
  (void)str;
  die("type::genIndexName called (INTERNAL ERROR)");
}
void Type::genMsgProxyName(XStr& str) {
  (void)str;
