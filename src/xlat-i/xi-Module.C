...
// DMK - Accel Support
  ...
  // If this is the main module, generate the general C file and include this modules
  // accel.h file
  if (isMain()) {
    XStr mainAccelStr_c;
...
// DMK - Accel Support
  ...
  if (!external) {
    ...
    // NOTE: Add a check so modules won't register more than once.  It is possible that to
    // modules
    //   could have 'extern' references to each other, creating infinite loops in the
    //   registration process.  Avoid this problem.
...
int Module::genAccels_spe_c_funcBodies(XStr& str) {
  ...
  if (isMain()) {
    str << "typedef void(*AccelFuncPtr)(DMAListEntry*);\n\n"
        ...
        << "FuncLookupTableEntry funcLookupTable[MODULE_" << name
        << "_FUNC_INDEX_COUNT];\n\n\n";
  }
  ...
  int rtn = 0;
  if (clist) {
    rtn += clist->genAccels_spe_c_funcBodies(str);
  }

  // Create the accelerated function registration function for accelerated entries local
  // to this module NOTE: Add a check so modules won't register more than once.  It is
  // possible that to modules
  //   could have 'extern' references to each other, creating infinite loops in the
  //   registration process.  Avoid this problem.
  ...
  if (isMain()) {
    ...
    str << "  if ((funcIndex >= SPE_FUNC_INDEX_USER) && (funcIndex < "
           "(SPE_FUNC_INDEX_USER + MODULE_"
        << name
        << "_FUNC_INDEX_COUNT))) {\n"
        ...
        << "    if (register_accel_funcs_" << name << "(0) != MODULE_" << name
        << "_FUNC_INDEX_COUNT) {\n"
        << "      printf(\"ERROR : register_accel_funcs_" << name
    ...
    str << "  } else if (funcIndex == SPE_FUNC_INDEX_CLOSE) {\n"
        ...
        << "  } else {\n"
        << "    printf(\"ERROR : Unknown funcIndex (%d) passed to funcLookup()... "
           "ignoring.\\n\", funcIndex);\n"
        << "  }\n";
...
void Module::genAccels_spe_c_regFuncs(XStr& str) {
  if (external) {
    ...
    if (clist) {
      clist->genAccels_spe_c_regFuncs(str);
    }
  }
}
...
void Module::genAccels_spe_c_callInits(XStr& str) {
  if (clist) {
    clist->genAccels_spe_c_callInits(str);
  }
}
...
void Module::genAccels_spe_h_includes(XStr& str) {
  ...
  if (clist) {
    clist->genAccels_spe_h_includes(str);
  }
}
...
void Module::genAccels_spe_h_fiCountDefs(XStr& str) {
  ...
  if (clist) {
    clist->genAccels_spe_h_fiCountDefs(str);
  }
}
...
void Module::genAccels_ppe_c_regFuncs(XStr& str) {
  if (external) {
    ...
    if (clist) {
      clist->genAccels_ppe_c_regFuncs(str);
    }
  }
}
