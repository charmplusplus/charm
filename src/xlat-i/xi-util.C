...
void XStr::spew(const char* b, const char* a1, const char* a2, const char* a3,
...
char* fortranify(const char* s, const char* suff1, const char* suff2, const char* suff3) {
  int i, len1 = strlen(s), len2 = strlen(suff1), len3 = strlen(suff2),
         len4 = strlen(suff3);
  ...
  strcat(str, suff2);
  strcat(str, suff3);
  for (i = 0; i < c + 1; i++) str[i] = tolower(str[i]);
  for (i = 0; i < c + 1; i++) strUpper[i] = toupper(str[i]);
  char* retVal;
...
const char* forWhomStr(forWhom w) {
  switch (w) {
    case forAll:
      return Prefix::Proxy;
    case forIndividual:
      return Prefix::ProxyElement;
    case forSection:
      return Prefix::ProxySection;
    case forIndex:
      return Prefix::Index;
    case forPython:
      return "";
...
void templateGuardEnd(XStr& str) { str << "#endif /* CK_TEMPLATES_ONLY */\n"; }
...
void _pretty_header(std::string type, std::string msg, int first_col, int last_col,
                    int first_line, int last_line) {
  std::cerr << cur_file << ":" << first_line << ":";

  if (first_col != -1) std::cerr << first_col << "-" << last_col << ": ";

  std::cerr << type << ": " << msg << std::endl;
}

void _pretty_print(std::string type, std::string msg, int first_col, int last_col,
                   int first_line, int last_line) {
  _pretty_header(type, msg, first_col, last_col, first_line, last_line);

  if (first_line <= inputBuffer.size() && first_line <= last_line &&
      first_col <= last_col) {
    ...
    if (err_line.length() != 0) {
      ...
      if (first_col != -1) std::cerr << "  " << caret_line;
      std::cerr << std::endl;
    }
  }
}

void pretty_msg(std::string type, std::string msg, int first_col, int last_col,
                int first_line, int last_line) {
...
void pretty_msg_noline(std::string type, std::string msg, int first_col, int last_col,
                       int first_line, int last_line) {
  if (first_line == -1) first_line = lineno;
  if (last_line == -1) last_line = lineno;
  _pretty_header(type, msg, first_col, last_col, first_line, last_line);
