			 ...
			 int replyLen,char *reply)
{
  ...
  { /*Compose a reply SHA-1 hash header*/
    CCS_AUTH_hash(securityMgr->getKey(securityMgr,attr),
...
static const char *CcsServer_createSalt(SOCKET fd,CCS_AUTH_clients *cl,
					CcsSecMan *securityMgr,CcsSecAttr *attr)
{
  ChMessageInt_t s1;
  ...
  if (CCS_AUTH_differ(securityMgr->getKey(securityMgr,attr),ChMessageInt(s2),
		      NULL,&s2hash))
    return "ERROR> CreateSalt client hash mismatch! (bad password?)";
  CCS_AUTH_hash(securityMgr->getKey(securityMgr,attr),ChMessageInt(s1),
		NULL,&reply.s1hash);
					...
					CcsMessageHeader *hdr)
{
  ...
  if (CCS_AUTH_differ(securityMgr->getKey(securityMgr,attr),salt,
		      hdr,&hash))
			...
			CcsSecAttr *attr,CcsMessageHeader *hdr) 
{
  ...
  */
  ...
  { /*Unauthenticated message-- do a security check*/
      ...
      if (!securityMgr->allowRequest(securityMgr,attr))
  ...
  { /*SHA-1 Authenticated request*/
	...
	return CcsServer_SHA1_message(fd,cl,securityMgr,attr,hdr);
      case 0x01: /*Request for salt*/
	return CcsServer_createSalt(fd,cl,securityMgr,attr);
      default: 
...
static CcsSecMan *globalSecurityMgr;
				     ...
				     CcsImplHeader *hdr,void **reqData)
{
  ...
  const char *err;
  if (NULL!=(err=CcsServer_readHeader(fd,&ccs_clientlist,globalSecurityMgr,
				      &hdr->attr,&req))) 
...
void CcsServer_sendReply(CcsImplHeader *hdr,int repBytes,const void *repData)
{
  ...
  CcsServer_writeReply(fd,globalSecurityMgr,&hdr->attr,repBytes,(char *)repData);
  skt_close(fd);
