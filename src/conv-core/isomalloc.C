...
static const constexpr memRange_t partition_granularity = 16u * meg;
...
{
  const uintptr_t available_slots = ((uintptr_t)end - (uintptr_t)start) / partition_granularity;
  CmiEnforce(available_slots >= numunits);
  return start + (available_slots * myunit / numunits * partition_granularity);
}
