/* 
Off-the-shelf glibc 2.2.5 malloc routine: useful because it's
   - Portable (should compile everywhere)
   - Compact (uses mmap for big buffers, can shift back sbrk)
   - Self-contained (this file is it)

This is slightly hacked as:
   - Use mm_* routine names, as defined below.
   - Maps charm config. macros to local macros, below.
   - Removed unused header file bp-checks.h (replace with macros).
   - Renamed malloc.h memory-gnu.h
   - Renamed threads-m.h memory-gnu-threads.h
   - Removed error-prone "memcpy" prototypes (use system header).
*/
...
/* Crashing bug caused by circular dependence:
 calloc calls arena_get2 calls pthread_setspecific calls calloc...
#ifdef CMK_SHARED_VARS_POSIX_THREADS_SMP
#  define MALLOC_HOOKS
#  define USE_PTHREADS
#endif
*/

#define RETURN_ADDRESS(x) (void *)x
...
#  include "malloc.h"
# else
#  include "memory-gnu.h"
# endif
#endif

/* #include <bp-checks.h> (use do-nothing versions below OSL 2003/8)
*/
