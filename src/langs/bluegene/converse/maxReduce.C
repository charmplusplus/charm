...
extern "C" void BgNodeStart(int argc, char **argv) {
  ...
  BgSendLocalPacket(COMPUTATION_ID, LARGE_WORK, sizeof(ComputeMsg), (char *)msg);
}

extern "C" void reduceHandler(char *info) {
  ...
  if ((i == 0) && (j == 0) && (k == 0)) {
	...
	max[i][j][k] = external_max;
      ...
      msg->max = max[i][j][k];
      BgSendPacket(i,j,k-1,REDUCE_HANDLER_ID,LARGE_WORK, sizeof(ReductionMsg), (char *)msg);
    }
  } else if ((i == 0) && (j == 0) && (k == Z_DIM - 1)) {
	...
	max[i][j][k] = external_max;
      ...
      ReductionMsg *msg = new ReductionMsg;
      msg->max = max[i][j][k];
      BgSendPacket(i,j-1,k,REDUCE_HANDLER_ID,LARGE_WORK, sizeof(ReductionMsg), (char *)msg);
    }
  } else if ((i == 0) && (j == Y_DIM - 1)) {
	...
	max[i][j][k] = external_max;
      ...
      ReductionMsg *msg = new ReductionMsg;
      msg->max = max[i][j][k];
      BgSendPacket(i-1,j,k,REDUCE_HANDLER_ID,LARGE_WORK, sizeof(ReductionMsg), (char *)msg);
    }
  } else if (i == X_DIM - 1) {
...
void computeMax(char *info) {
  ...
  ReductionMsg *msg = new ReductionMsg;
  msg->max = max;
  BgSendLocalPacket(REDUCE_HANDLER_ID, LARGE_WORK, sizeof(ReductionMsg), (char *)msg);
