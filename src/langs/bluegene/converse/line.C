...
extern "C" void BgGlobalInit(int argc, char **argv)
{
  if (argc < 7) { 
    CmiAbort("Usage: line2 <x> <y> <z> <numCommTh> <numWorkTh> <numIter> \n"); 
  }
...
extern "C" void BgNodeStart(int argc, char **argv)
{
  ...
  if(x==0 && y==0 && z==0) 
  {
    Msg* msg = new Msg;
      	msg->type = X_NODE;
    BgSendLocalPacket(BCastID, LARGE_WORK, sizeof(Msg),(char*)msg);
  }
...
extern "C" void BCastHandler(char* info) {
  int i, j, k;
  BgGetXYZ(&i, &j, &k);
  ...
  case X_NODE:	//possible only on X-axis
	if(i!=numBgX-1) 	//send message further down X with type X_NODE
	  BgSendPacket(i+1, j, k, BCastID, LARGE_WORK, sizeof(Msg), (char*)msg);

  	//send message down Y with type Y_NODE
	msg = new Msg;
	  msg->type = Y_NODE;
	BgSendPacket(i, j+1, k, BCastID, LARGE_WORK, sizeof(Msg), (char*)msg);

  	//send message down Z with type Z_NODE
	msg = new Msg;
	  msg->type = Z_NODE;
	BgSendPacket(i, j, k+1, BCastID, LARGE_WORK, sizeof(Msg), (char*)msg);
  ...
  case Z_NODE:
	...
	{
	  msg->time = BgGetThreadTime();
  	  //ckout <<"Time Message sent from Node "<<i<<" "<<j<<" "<<k
	  //      <<" at time "<<msg->time<<endl;
	  BgSendLocalPacket(TimeID, SMALL_WORK, sizeof(Msg), (char*)msg);
	}
  	break;
  }
}

extern "C" void TimeHandler(char* info) 
{
  int i, j, k;
  BgGetXYZ(&i, &j, &k);
  ...
  if(ud->expected == ud->received)
  {
  	msg->time = ud->maxTime;
   	switch(ud->expected) {
	  case 1:
		BgSendPacket(i, j-1, k, TimeID, SMALL_WORK, sizeof(Msg), (char*)msg);
	  	break;
	  case 2:
	  	if(j == 0)
		  BgSendPacket(i-1, j, k, TimeID, SMALL_WORK, sizeof(Msg), (char*)msg);
		else
		  BgSendPacket(i, j-1, k, TimeID, SMALL_WORK, sizeof(Msg), (char*)msg);
	  	break;
	  case 3:
	  	if(i == 0)
		{
		  ...
		  ud->iter += 1;
		  ud->startTime[ud->iter] = BgGetThreadTime();
		  //ckout << "Iteration No " << ud->iter << ", start time " << ud->startTime[ud->iter] << endl;
		  msg->type = X_NODE;
		  BgSendPacket(0, 0, 0, BCastID, LARGE_WORK, sizeof(Msg), (char*)msg);
		}
		else
		  BgSendPacket(i-1, j, k, TimeID, SMALL_WORK, sizeof(Msg), (char*)msg);
	  	break;
