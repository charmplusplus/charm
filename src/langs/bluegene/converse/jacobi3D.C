//
// 3D Jacobi code for the Blue Gene simulator
...
extern "C" void BgNodeStart(int argc, char **argv) 
{
  /*
  ...
  */
  ...
  BgSendLocalPacket(computeID, LARGE_WORK, sizeof(computeMsg), (char *)msg);

  BgSetNodeData((char *)nd);
}


extern "C" void compute(char *info) 
{
  ...
  */
  ...
  if ((x == 0) && (y == 0) && (z == 0)) {
    BgSendLocalPacket(reduceID, SMALL_WORK, sizeof(reduceMsg), (char *)msg);
  } else {
    BgSendPacket(0,0,0,reduceID, SMALL_WORK, sizeof(reduceMsg), (char *)msg);
  }
  
}

extern "C" void ghostexchange(char *info) {
  int x,y,z;
  BgGetXYZ(&x,&y,&z);
  ...
  if (x != 0) {
	...
	for (int k=0; k < z_size; k++) {
	  ...
	  if ((localcount == NUM_DBLMSG_COUNT) ||
	    ...
	    BgSendPacket(x-1,y,z,ghostID,SMALL_WORK, sizeof(ghostMsg), (char *)x1_msg);
	    localcount = 0;
	  }
	}
      }
  ...
  if (x != numBgX - 1) {
	...
	for (int k=0; k < z_size; k++) {
	  ...
	  if ((localcount == NUM_DBLMSG_COUNT) ||
	    ...
	    BgSendPacket(x+1,y,z,ghostID,SMALL_WORK, sizeof(ghostMsg), (char *)x2_msg);
	    localcount = 0;
	  }
	}
      }
  ...
  if (y != 0) {
	...
	for (int k=0; k < z_size; k++) {
	  ...
	  if ((localcount == NUM_DBLMSG_COUNT) ||
	    ...
	    BgSendPacket(x,y-1,z,ghostID,SMALL_WORK, sizeof(ghostMsg), (char *)y1_msg);
	    localcount = 0;
	  }
	}
      }
  ...
  if (y != numBgY - 1) {
	...
	for (int k=0; k < z_size; k++) {
	  ...
	  if ((localcount == NUM_DBLMSG_COUNT) ||
	    ...
	    BgSendPacket(x,y+1,z,ghostID,SMALL_WORK, sizeof(ghostMsg), (char *)y2_msg);
	    localcount = 0;
	  }
	}
      }
  ...
  if (z != 0) {
	...
	for (int k=0; k < z_size; k++) {
	  ...
	  if ((localcount == NUM_DBLMSG_COUNT) ||
	    ...
	    BgSendPacket(x,y,z-1,ghostID,SMALL_WORK, sizeof(ghostMsg), (char *)z1_msg);
	    localcount = 0;
	  }
	}
      }
  ...
  if (z != numBgZ - 1) {
	...
	for (int k=0; k < z_size; k++) {
	  ...
	  if ((localcount == NUM_DBLMSG_COUNT) ||
	    ...
	    BgSendPacket(x,y,z+1,ghostID,SMALL_WORK, sizeof(ghostMsg), (char *)z2_msg);
	    localcount = 0;
	  }
	}
      }
...
extern "C" void reduce(char *info) 
{
  ...
  if (localdata->reduction_count == 0) {
	...
	for (int j=0; j<numBgY;j++) {
	  for (int k=0; k<numBgZ;k++) {
	    ...
	    if ((i == 0) && (j == 0) && (k == 0)) {
	      BgSendLocalPacket(exchangeID, LARGE_WORK, sizeof(exchangeMsg),(char *)msg);
	    } else {
	      BgSendPacket(i,j,k,exchangeID,SMALL_WORK, sizeof(exchangeMsg),(char *)msg);
	    }
	  }
	}
      }
	...
	for (int j=0; j<numBgY;j++) {
	  for (int k=0; k<numBgZ;k++) {
	    ...
	    if ((i == 0) && (j == 0) && (k == 0)) {
	      BgSendLocalPacket(outputID, LARGE_WORK, sizeof(outputMsg), (char *)msg);
	    } else {
	      BgSendPacket(i,j,k,outputID,SMALL_WORK, sizeof(outputMsg), (char *)msg);
	    }
	  }
	}
      }
...
extern "C" void ghostrecv(char *info)
{
  int x,y,z;
  BgGetXYZ(&x,&y,&z);
  ...
  */
  ...
  if ((x1_total == localdata->ghost_x1_elements_current) &&
    ...
    computeMsg *msg = new computeMsg;
    BgSendLocalPacket(computeID, LARGE_WORK, sizeof(computeMsg), (char *)msg); // get to work!
  }
}

extern "C" void outputData(char *info) {
  int x,y,z;
  BgGetXYZ(&x,&y,&z);
