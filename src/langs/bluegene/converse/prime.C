...
{
  if (argc < 7) { 
    CmiAbort("Usage: <program> <x> <y> <z> <numCommTh> <numWorkTh> <range>\n"); 
  }
  ...
  if(max-min>0)
  {
	...
	{	
	  ...
	  if(msg->min>=max) { msg->min=msg->max=0; }			
  	  ...
  	  BgSendLocalPacket(computeNumPrimeID, LARGE_WORK, sizeof(Msg), (char *)msg);
 ...
 */
extern "C" void computeNumPrime(char *info) 
{
  int x,y,z;
  BgGetXYZ(&x,&y,&z);
  ...
  msg->pc = pc;
  BgSendLocalPacket(contributeID,LARGE_WORK, sizeof(Msg), (char *)msg);
}
 ...
 */
extern "C" void contribute(char *info)
{
  int x,y,z;
  BgGetXYZ(&x,&y,&z);
  ...
  if(ud->count==reqCount)  //if data for reduction is ready
  {
    Msg *msg = (Msg*)info ;
    msg->pc = ud->pc;
    BgSendLocalPacket(reduceID, LARGE_WORK, sizeof(Msg), (char *)msg);
    return ;
  }
//  else
//    delete (Msg*)info;
...
{
  int pc = ((userData*)(BgGetNodeData()))->pc;
...
//  delete (Msg*)info;
  ...
  Msg *msg = (Msg*)info;
  msg->pc = pc;
  BgSendPacket(x,y,z, contributeID, LARGE_WORK, sizeof(Msg), (char *)msg);
}
